<div class="container">
	<h2 class="judul">Selling Rate(Air)</h2>
	<div id="alert"></div>
	<form class="form-horizontal">
		...
		<div class="row">
			...
			<div class="form-actions btn-group">
				<?php if( in_array( 'create_selling_rate_air', unserialize($capabilities) ) ){ ?>
				...
				<?php if( in_array( 'update_selling_rate_air', unserialize($capabilities) ) ){ ?>
				<a href="#" class="btn btn-primary ubah" style="top:-15px"><i class="icon-pencil icon-white"></i> Update</a>
				<?php } ?>
				<?php if( intval($role)!= 1){ ?>
					<?php if( in_array( 'delete_selling_rate_air', unserialize($capabilities) ) ){ ?>
...
function initializex(){
	...
	$('#cetak').attr('href', 'pdf');
	$('.selling-rate-air-row').remove();
	$( '#transaksix' ).append(
		add_row_selling_rate_air( 0, 0 )
	);
	var d = new Date();
	var tgl = d.getDate();
	if( tgl < 10 ) tgl = "0" + tgl;
...
function bookingBaris(num){
	var tgldatefrom = $( '#selling-rate-air-'+num+'-date_from').val();
	var tgldateuntil = $( '#selling-rate-air-'+num+'-date_until').val();
...
function get_buying_rate(num)
{
	...
	var charges_code = $('#selling-rate-air-'+num+'-charges_code').val();
	var port_awal = $('#selling-rate-air-'+num+'-code_awal').val();
	var port_akhir = $('#selling-rate-air-'+num+'-code_akhir').val();
	if(tglawal!="" && tglakhir!="" && cusid!="" && charges_code!="" && port_awal!="" && port_akhir!="")
	{
		$.post( 'load_buying_rate', { tgl_from: tglawal , tgl_until: tglakhir , charges: charges_code , cus_id: cusid , portawal : port_awal, portakhir: port_akhir }, function( result ){
			if( result === '' ||  data===null){
				...
				$('#alert').html( message );
				$('#selling-rate-air-'+num+'-cus_id').focus();
				var buying_rate= $('#selling-rate-air-'+num+'-buying_rate').val();
				var base_rate= $('#selling-rate-air-'+num+'-base_rate').val();
				var selling_rate= $('#selling-rate-air-'+num+'-selling_rate').val();
				$('#selling-rate-air-'+num+'-buying_rate').val('');
				$('#selling-rate-air-'+num+'-base_rate').val('');
				$('#selling-rate-air-'+num+'-selling_rate').val('');
			}else{
				...
				console.log(data);
				$('#alert').html( "" );	
				var base_margin= $('#selling-rate-air-'+num+'-base_margin').val();
				var additional_margin= $('#selling-rate-air-'+num+'-additional_margin').val();
				$('#selling-rate-air-'+num+'-buying_rate').val(data['selling_rate']);
				$('#selling-rate-air-'+num+'-base_rate').val(parseFloat( base_margin)+parseFloat( data['selling_rate']));
				$('#selling-rate-air-'+num+'-selling_rate').val(parseFloat( base_margin)+parseFloat( data['selling_rate'])+parseFloat(additional_margin));
				$('#selling-rate-air-'+num+'-unit_code').val(data['unit_code']);
				$('#selling-rate-air-'+num+'-unit_description').val(data['unit_description']);
				$('#selling-rate-air-'+num+'-currency_code').val(data['currency_code']);
				$('#selling-rate-air-'+num+'-currency_description').val(data['currency_description']);
			}
		});
	}
...
function add_row_selling_rate_air( i, action ){
	var tabel = '<tr class="selling-rate-air-row" id="selling-rate-air-'+i+'">';

	tabel += '<td><div class="input-append"><input style="width:80px" type="text" name="selling_rate_air['+i+'][date_from]" id="selling-rate-air-'+i+'-date_from" class="selling-rate-air-date_from tanggal" /></td>';
	...
	tabel += '<td><input style="width:100px;text-align:right" type="text" name="selling_rate_air['+i+'][buying_rate]" id="selling-rate-air-'+i+'-buying_rate" class="selling-rate-air-buying_rate currency detail_jadwal" value="0" /></td>';
	
	tabel += '<td><input style="width:100px;text-align:right" type="text" name="selling_rate_air['+i+'][base_margin]" id="selling-rate-air-'+i+'-base_margin" class="selling-rate-air-base_margin currency detail_jadwal" value="0" /></td>';
	
	tabel += '<td><input style="width:100px;text-align:right" readonly type="text" name="selling_rate_air['+i+'][base_rate]" id="selling-rate-air-'+i+'-base_rate" class="selling-rate-air-base_rate currency detail_jadwal" value="0" /></td>';
	
	tabel += '<td><input style="width:100px;text-align:right" type="text" name="selling_rate_air['+i+'][additional_margin]" id="selling-rate-air-'+i+'-additional_margin" class="selling-rate-air-additional_margin currency detail_jadwal" value="0" /></td>';
	
	tabel += '<td><input style="width:100px;text-align:right" readonly type="text" name="selling_rate_air['+i+'][selling_rate]" id="selling-rate-air-'+i+'-selling_rate" class="selling-rate-air-selling_rate currency detail_jadwal" value="0" /></td>';
	
	if( action === 1 ){
		tabel += '<td class="action" id="action-baris-selling-rate-air-'+i+'"><a href="#void()" class="tombol tombol-small hapus-baris-selling-rate-air detail_jadwal" id="hapus-baris-selling-rate-air-'+i+'"><i class="icon-remove"></i></a></td>';
	} else tabel += '<td class="action" id="action-baris-selling-rate-air-'+i+'"><a href="#void()" class="tombol tombol-small tambah-baris-selling-rate-air detail_jadwal" id="tambah-baris-selling-rate-air-'+i+'"><i class="icon-plus"></i></a></td>';
	tabel += '</tr>';
	$( "#baris-selling-rate-air" ).val( i + 1 );
	return tabel;
}
$( document ).ready( function() {
	...
	$('.selling-rate-air-buying_rate').live( 'keyup', function(e){
		var base_rate = 0;
		var selling_rate = 0;
		var asal = $( this ).attr( 'id' );
		var base_rate =	Number($( '#'+asal.replace( 'buying_rate', 'base_margin' ) ).val().replace( /(?!^-)[^0-9.]+/g, "" ) ) + Number( $( this ).val().replace(  /(?!^-)[^0-9.]+/g, "" ) );
		var selling_rate =	Number($( '#'+asal.replace( 'buying_rate', 'base_margin' ) ).val().replace( /(?!^-)[^0-9.]+/g, "" ) ) + Number( $( this ).val().replace(  /(?!^-)[^0-9.]+/g, "" ) ) + Number($( '#'+asal.replace( 'buying_rate', 'additional_margin' ) ).val().replace( /(?!^-)[^0-9.]+/g, "" ) );
		
		$('#'+asal.replace( 'buying_rate', 'base_rate' ) ).val(base_rate).formatCurrency({ colorize: true, negativeFormat: '-%s%n', roundToDecimalPlace: 2 });
		$('#'+asal.replace( 'buying_rate', 'selling_rate' ) ).val(selling_rate).formatCurrency({ colorize: true, negativeFormat: '-%s%n', roundToDecimalPlace: 2 });
	});
	

	$('.selling-rate-air-base_margin').live( 'keyup', function(e){
		var base_rate = 0;
		var selling_rate = 0;
		var asal = $( this ).attr( 'id' );
	...
	$('.selling-rate-air-additional_margin').live( 'keyup', function(e){
		var selling_rate = 0;
		var asal = $( this ).attr( 'id' );
		
		var selling_rate =	Number($( '#'+asal.replace( 'additional_margin', 'base_rate' ) ).val().replace( /(?!^-)[^0-9.]+/g, "" ) ) + Number( $( this ).val().replace(  /(?!^-)[^0-9.]+/g, "" ) );
		
		$('#'+asal.replace( 'additional_margin', 'selling_rate' ) ).val(selling_rate).formatCurrency({ colorize: true, negativeFormat: '-%s%n', roundToDecimalPlace: 2 });
	});
	...
	$( '.detail_jadwal' ).live( 'keydown', function( e ){
		...
		if ( e.which === kodehapusbaris ){
			e.preventDefault();
			var xobj = document.getElementById('hapus-baris-selling-rate-air-'+num);
			if (xobj != null){
				$( '#hapus-baris-selling-rate-air-'+( Number( num ) ) ).click();
			}else{
				$("input[id*='" + "selling-rate-air-"+num + "']").val('');
			}
			setfocus(Number(num),'id','selling-rate-air');
		}
	});
	
	$( '.selling-rate-air-id' ).live( 'keydown', function( e ){
		...
		if ( e.which === 13 ){
			...
			e.preventDefault;
			setfocus(Number(num)+1,'id','selling-rate-air');
		}else if ( e.which === 38 ){
			e.preventDefault;
			setfocus2(Number(num),'id','selling-rate-air');
		}else if ( e.which === 39 ){
			...
			e.preventDefault();
				setfocus(Number(num),'charges_code','selling-rate-air');
			}
		}else if ( e.which === 37 ){
			if ($(this).caret().start===0){
			e.preventDefault();
				setfocus2((Number(num)),'currency_code','selling-rate-air');
			}
		}
	});
	
	
	$( '.selling-rate-air-date_from' ).live( 'keydown', function( e ){
		...
		if ( e.which === 13 ){
			e.preventDefault;
			bookingBaris(num);
			setfocus(Number(num),'date_until','selling-rate-air');
		}else if ( e.which === 40 ){
			e.preventDefault;
			setfocus(Number(num)+1,'date_from','selling-rate-air');
		}else if ( e.which === 38 ){
			e.preventDefault;
			setfocus2(Number(num),'date_from','selling-rate-air');
		}else if ( e.which === 39 ){
			var textBox = document.getElementById($( this ).attr( 'id' ));
			var textLength = textBox.value.length;			
			if (($(this).val()==='') || (getselectedtext($( this ).attr( 'id' ))==textBox.value) || ($(this).caret().start===textBox.value.length)){
			...
			e.preventDefault();
			bookingBaris(num);
			setfocus2((Number(num)),'additional_margin','selling-rate-air');
			}
		}
	});
	
	$( '.selling-rate-air-date_from' ).live( 'blur', function(){
		var asal = $( this ).attr( 'id' );
		var num = asal.split( '-' )[ 3 ];
		bookingBaris(num);
	});
	
	$( '.selling-rate-air-date_until' ).live( 'keydown', function( e ){
		...
		if ( e.which === 13 ){
			...
			e.preventDefault;
			setfocus(Number(num)+1,'date_until','selling-rate-air');
		}else if ( e.which === 38 ){
			e.preventDefault;
			setfocus2(Number(num),'date_until','selling-rate-air');
		}else if ( e.which === 39 ){
			var textBox = document.getElementById($( this ).attr( 'id' ));
			var textLength = textBox.value.length;			
			if (($(this).val()==='') || (getselectedtext($( this ).attr( 'id' ))==textBox.value) || ($(this).caret().start===textBox.value.length)){
			...
			e.preventDefault();
			bookingBaris(num);
			setfocus((Number(num)),'date_from','selling-rate-air');
			}
		}
	});
	
	$( '.selling-rate-air-date_until' ).live( 'blur', function(){
		var asal = $( this ).attr( 'id' );
		var num = asal.split( '-' )[ 3 ];
		bookingBaris(num);
	});
	
	$( '.selling-rate-air-charges_code' ).live( 'blur', function(){
		var asal = $( this ).attr( 'id' );
		var num = asal.split( '-' )[ 3 ];
		if($( this ).val()==''){
	...
	$( '.selling-rate-air-charges_code' ).live( 'keydown', function( e ){
		...
		if ( e.which === 13 ){
			...
			e.preventDefault;
			setfocus(Number(num)+1,'charges_code','selling-rate-air');
		}else if ( e.which === 38 ){
			e.preventDefault;
			setfocus2(Number(num),'charges_code','selling-rate-air');
		}else if ( e.which === 39 ){
			var textBox = document.getElementById($( this ).attr( 'id' ));
			var textLength = textBox.value.length;			
			if (($(this).val()==='') || (getselectedtext($( this ).attr( 'id' ))==textBox.value) || ($(this).caret().start===textBox.value.length)){
			e.preventDefault();
				setfocus(Number(num),'code_awal','selling-rate-air');
			}
		}else if ( e.which === 37 ){
			if ($(this).caret().start===0){
			e.preventDefault();
	...
	$( '.selling-rate-air-code_awal' ).live( 'blur', function(){
		var asal = $( this ).attr( 'id' );
		var num = asal.split( '-' )[ 3 ];
		if($( this ).val()==''){
	...
	$( '.selling-rate-air-code_awal' ).live( 'keydown', function( e ){
		...
		if ( e.which === 13 ){
			...
			e.preventDefault;
			setfocus(Number(num)+1,'code_awal','selling-rate-air');
		}else if ( e.which === 38 ){
			e.preventDefault;
			setfocus2(Number(num),'code_awal','selling-rate-air');
		}else if ( e.which === 39 ){
			var textBox = document.getElementById($( this ).attr( 'id' ));
			var textLength = textBox.value.length;			
			if (($(this).val()==='') || (getselectedtext($( this ).attr( 'id' ))==textBox.value) || ($(this).caret().start===textBox.value.length)){
			e.preventDefault();
				setfocus(Number(num),'code_akhir','selling-rate-air');
			}
		}else if ( e.which === 37 ){
			if ($(this).caret().start===0){
			e.preventDefault();
	...
	$( '.selling-rate-air-code_akhir' ).live( 'blur', function(){
		var asal = $( this ).attr( 'id' );
		var num = asal.split( '-' )[ 3 ];
		if($( this ).val()==''){
	...
	$( '.selling-rate-air-code_akhir' ).live( 'keydown', function( e ){
		...
		if ( e.which === 13 ){
			...
			e.preventDefault;
			setfocus(Number(num)+1,'code_akhir','selling-rate-air');
		}else if ( e.which === 38 ){
			e.preventDefault;
			setfocus2(Number(num),'code_akhir','selling-rate-air');
		}else if ( e.which === 39 ){
			var textBox = document.getElementById($( this ).attr( 'id' ));
			var textLength = textBox.value.length;			
			if (($(this).val()==='') || (getselectedtext($( this ).attr( 'id' ))==textBox.value) || ($(this).caret().start===textBox.value.length)){
			e.preventDefault();
				setfocus(Number(num),'unit_code','selling-rate-air');
			}
		}else if ( e.which === 37 ){
			if ($(this).caret().start===0){
			e.preventDefault();
	...
	$( '.selling-rate-air-unit_code' ).live( 'blur', function(){
		var asal = $( this ).attr( 'id' );
		var num = asal.split( '-' )[ 3 ];
		get_buying_rate(num);
		if($( this ).val()==''){
	...
	$( '.selling-rate-air-unit_code' ).live( 'keydown', function( e ){
		...
		if ( e.which === 13 ){
			...
			e.preventDefault;
			setfocus(Number(num)+1,'unit_code','selling-rate-air');
		}else if ( e.which === 38 ){
			e.preventDefault;
			setfocus2(Number(num),'unit_code','selling-rate-air');
		}else if ( e.which === 39 ){
			var textBox = document.getElementById($( this ).attr( 'id' ));
			var textLength = textBox.value.length;			
			if (($(this).val()==='') || (getselectedtext($( this ).attr( 'id' ))==textBox.value) || ($(this).caret().start===textBox.value.length)){
			e.preventDefault();
				setfocus(Number(num),'currency_code','selling-rate-air');
			}
		}else if ( e.which === 37 ){
			if ($(this).caret().start===0){
			e.preventDefault();
	...
	$( '.selling-rate-air-currency_code' ).live( 'blur', function(){
		var asal = $( this ).attr( 'id' );
		var num = asal.split( '-' )[ 3 ];
		get_buying_rate(num);
		if($( this ).val()==''){
	...
	$( '.selling-rate-air-currency_code' ).live( 'keydown', function( e ){
		...
		if ( e.which === 13 ){
			...
			e.preventDefault;
			setfocus(Number(num)+1,'currency_code','selling-rate-air');
		}else if ( e.which === 38 ){
			e.preventDefault;
			setfocus2(Number(num),'currency_code','selling-rate-air');
		}else if ( e.which === 39 ){
			...
			if (($(this).val()==='') || (getselectedtext($( this ).attr( 'id' ))==textBox.value) || ($(this).caret().start===textBox.value.length)){
			e.preventDefault();
				setfocus(Number(num),'buying_rate','selling-rate-air');
			}
		}else if ( e.which === 37 ){
			if ($(this).caret().start===0){
			e.preventDefault();
	...
	$( '.selling-rate-air-buying_rate' ).live( 'keydown', function( e ){
	
		var asal = $( this ).attr( 'id' );
		var num = asal.split( '-' )[ 3 ];
		if ( e.which === 13 ){
			e.preventDefault;
			setfocus(Number(num),'base_margin','selling-rate-air');
		}else if ( e.which === 40 ){
			e.preventDefault;
			setfocus(Number(num)+1,'buying_rate','selling-rate-air');
		}else if ( e.which === 38 ){
			e.preventDefault;
			setfocus2(Number(num),'buying_rate','selling-rate-air');
		}else if ( e.which === 39 ){
			var textBox = document.getElementById($( this ).attr( 'id' ));
			var textLength = textBox.value.length;			
			if (($(this).val()==='') || (getselectedtext($( this ).attr( 'id' ))==textBox.value) || ($(this).caret().start===textBox.value.length)){
	...
	$( '.selling-rate-air-base_margin' ).live( 'keydown', function( e ){
	
		var asal = $( this ).attr( 'id' );
		var num = asal.split( '-' )[ 3 ];
		if ( e.which === 13 ){
			e.preventDefault;
			setfocus(Number(num),'additional_margin','selling-rate-air');
		}else if ( e.which === 40 ){
			e.preventDefault;
			setfocus(Number(num)+1,'base_margin','selling-rate-air');
		}else if ( e.which === 38 ){
			e.preventDefault;
			setfocus2(Number(num),'base_margin','selling-rate-air');
		}else if ( e.which === 39 ){
			var textBox = document.getElementById($( this ).attr( 'id' ));
			var textLength = textBox.value.length;			
			if (($(this).val()==='') || (getselectedtext($( this ).attr( 'id' ))==textBox.value) || ($(this).caret().start===textBox.value.length)){
	...
	$( '.selling-rate-air-additional_margin' ).live( 'keydown', function( e ){
	
		var asal = $( this ).attr( 'id' );
		var num = asal.split( '-' )[ 3 ];
		if ( e.which === 13 ){
			e.preventDefault;
			if( Number( $( '#baris-selling-rate-air' ).val() ) === ( Number( num ) + 1 ) ){
				$( '#tambah-baris-selling-rate-air-'+num ).click();
				nomorbaru();
			}
			setfocus(Number(num)+1,'date_from','selling-rate-air');
		}else if ( e.which === 40 ){
			e.preventDefault;
			setfocus(Number(num)+1,'additional_margin','selling-rate-air');
		}else if ( e.which === 38 ){
			e.preventDefault;
			setfocus2(Number(num),'additional_margin','selling-rate-air');
		}else if ( e.which === 39 ){
	...
	$( '.selling-rate-air-additional_margin' ).live( 'keydown', function( e ){
		if ( e.which === 13 ){
			...
			if( $( this ).val() !== '' ){
				var asal = $( this ).attr( 'id' );
				var num = asal.split( '-' )[ 3 ];
				console.log( $( '#baris-selling-rate-air' ).val() == ( Number( num ) + 1 ) );
				if( Number( $( '#baris-selling-rate-air' ).val() ) === ( Number( num ) + 1 ) ){
					$( '#tambah-baris-selling-rate-air-'+num ).click();
					nomorbaru();
				}
				$( '#selling-rate-air-'+( Number( num ) + 1 )+'-id' ).focus();
			}
		}
	});
		...
		if($(this).val()!==$(this).attr('oldvalue')){
			$.post( 'db_read', { cus_id: $( '#cus_id' ).val() }, function( result ){
				if( result === '' ){
					...
					var bln1 = tanggal1[ 1 ];
					var thn1 = tanggal1[ 0 ];
						$( '#transaksix' ).append( add_row_selling_rate_air( i, 1 ) );
						$( '#selling-rate-air-'+i+'-id' ).val( data['jadwal'][i][ 'id' ] );
						$( '#selling-rate-air-'+i+'-date_from' ).val( tgl + "-" + bln + "-" + thn );
						$( '#selling-rate-air-'+i+'-date_until' ).val( tgl1 + "-" + bln1 + "-" + thn1 );
						$( '#selling-rate-air-'+i+'-charges_code' ).val( data['jadwal'][i][ 'charges_code' ] );
						$( '#selling-rate-air-'+i+'-charges_description' ).val( data['jadwal'][i][ 'charges_description' ] );
						$( '#selling-rate-air-'+i+'-code_awal' ).val( data['jadwal'][i][ 'code_awal' ] );
						$( '#selling-rate-air-'+i+'-port_awal' ).val( data['jadwal'][i][ 'port_awal' ] );
						$( '#selling-rate-air-'+i+'-code_akhir' ).val( data['jadwal'][i][ 'code_akhir' ] );
						$( '#selling-rate-air-'+i+'-port_akhir' ).val( data['jadwal'][i][ 'port_akhir' ] );
						$( '#selling-rate-air-'+i+'-unit_code' ).val( data['jadwal'][i][ 'unit_code' ] );
						$( '#selling-rate-air-'+i+'-unit_description' ).val( data['jadwal'][i][ 'unit_description' ] );
						$( '#selling-rate-air-'+i+'-currency_code' ).val( data['jadwal'][i][ 'currency_code' ] );
						$( '#selling-rate-air-'+i+'-currency_description' ).val( data['jadwal'][i][ 'currency_description' ] );
						$( '#selling-rate-air-'+i+'-buying_rate' ).val( parseFloat( data['jadwal'][i][ 'buying_rate' ] ) ).formatCurrency({ colorize: true, negativeFormat: '-%s%n', roundToDecimalPlace: 2 });
						$( '#selling-rate-air-'+i+'-base_margin' ).val( parseFloat( data['jadwal'][i][ 'base_margin' ] ) ).formatCurrency({ colorize: true, negativeFormat: '-%s%n', roundToDecimalPlace: 2 });
						$( '#selling-rate-air-'+i+'-base_rate' ).val( parseFloat( data['jadwal'][i][ 'base_rate' ] ) ).formatCurrency({ colorize: true, negativeFormat: '-%s%n', roundToDecimalPlace: 2 });
						$( '#selling-rate-air-'+i+'-additional_margin' ).val( parseFloat( data['jadwal'][i][ 'additional_margin' ] ) ).formatCurrency({ colorize: true, negativeFormat: '-%s%n', roundToDecimalPlace: 2 });
						$( '#selling-rate-air-'+i+'-selling_rate' ).val( parseFloat( data['jadwal'][i][ 'selling_rate' ] ) ).formatCurrency({ colorize: true, negativeFormat: '-%s%n', roundToDecimalPlace: 2 });
						$( '#baris-selling-rate-air' ).val( i + 1 );
					}
					$( '#transaksix' ).append( add_row_selling_rate_air( i, 0 ) );
	...
	$( '#cus_id' ).bind( 'keydown', function( e ) {
		if ( e.which === 13 ){
			e.preventDefault();
			if( $( this ).val() !== '' ){	
				setfocus(0,'date_from','selling-rate-air');
			}
		}
	});
	...
	$( '.tambah-baris-selling-rate-air' ).live( 'click', function(){
		var str = $( this ).attr( 'id' );
		var num = str.replace( "tambah-baris-selling-rate-air-", "" );

		// Change action
		$( "#action"+str.replace( "tambah", "" ) ).html(
			'<a href="#void()" class="tombol tombol-small hapus-baris hapus-baris-selling-rate-air" id="hapus-baris-selling-rate-air-'+num+'"><i class="icon-remove"></i></a>'
		);

		// Insert row
		$( '#transaksix' ).append(
			add_row_selling_rate_air( Number( str.replace( 'tambah-baris-selling-rate-air-', '' ) ) + 1, 0 )
		);
		setfocus(Number(num)+1,'id','selling-rate-air');
	});

	$( '.hapus-baris-selling-rate-air' ).live( 'click', function(){
		...
		if( $( '#cus_id' ).val() !== '' ){
			$.post( 'db_create', $( 'form' ).serialize(), function( result ){
				...
				if( success['status'] ==='Error' ){
					...
					$('#alert').html( message );
					//$( '#cetak' ).attr( 'href', 'pdf?cus_id=' + $('#cus_id').val());
					initializex();
				} else {
