<div class="container">
	<h2 class="judul">Carrier Booking (SEA)</h2>
	<div id="alert"></div>
	<form class="form-horizontal">
		<fieldset>
			<div class="row">
				<div class="control-group span6" style="height:15px">
					<label class="control-label" for="si_no">SI No.</label>
					<div class="controls">
						<div class="input-append"><input type="text" siref="" name="si_no" id="si_no" oldvalue="" browseobj="cari_si_no" /><a style="display:none" class="add-on browse" id="cari_si_no" href="cari?ref=si_no&tipe=carrier_booking_sea" title="Klik untuk mencari SI number"><i class="icon-search"></i></a></div>
					</div>
				</div>
				<div class="control-group span6" style="height:15px">
					<label class="control-label" for="order_date">Date</label>
					<div class="controls">
			...
			<div class="row">
				<div class="control-group span6" style="height:15px">
					...
					<div class="controls">
						<input type="text" maxlength="30" name="reference" id="reference" />
					</div>
				</div>
			</div>
			<div class="row">
				<div class="control-group span9">
					<div class="controls">
						<ul class="nav nav-tabs nav-justified " id="meta">
...
<!--							<li><a href="#biaya" class="tab" title="Klik untuk mengisi informasi biaya lain">B. Lain</a></li>-->
			...
			<div class="tab-content ">
				<div class="span12 tab-pane active" style="height:300px;width:1300px" id="parties">
					<div class="row">
						...
						<div class="control-group span6">
							<label class="control-label" for="cnee_id">Consignee ID :</label>
							<div class="controls">
				...
				<div class="span12 tab-pane portvessel" style="height:300px;width:1300px" id="portvessel">
					...
					<div class="row">
						<div class="control-group span6" style="height:15px">
							...
							<div class="controls">
								<div class="input-append"><input style="width:110px" type="text" name="carrier" id="carrier" oldvalue="" browseobj="cari_carrier" /><a style="display:none" class="add-on browse" id="cari_carrier" href="cari?ref=carrier&tipe=customer" title="Klik untuk mencari id carrier"><i class="icon-search"></i></a></div>
								<input type="text" readonly="readonly"  name="carrier_name" id="carrier_name"/>
				...
				<div class="span12 tab-pane shipment" style="height:300px;width:1300px" id="shipment">
					...
					<div class="row">
						<div class="control-group span12" >
							...
							<table class="table">
								<thead height="5px">
									<tr height="5px"><th style="width:100px">Container Code</th><th style="width:100px">Size</th><th style="width:100px">Type</th><th style="width:100px">Seal No.</th><th style="width:100px">Gross Weight</th><th style="width:100px">Nett Weight</th><th style="width:100px">Measurement</th><th style="width:100px">Description</th><th></th></tr>
								</thead>
			...
			<div class="span12 form-actions btn-group">
				<?php if( in_array( 'create_carrier_booking_sea', unserialize($capabilities) ) ){ ?>
				...
				<?php if( in_array( 'update_carrier_booking_sea', unserialize($capabilities) ) ){ ?>
				<a href="#" class="btn btn-primary ubah" style="top:-15px"><i class="icon-pencil icon-white"></i> Ubah</a>
				<?php } ?>
				<?php if( intval($role)!= 1){ ?>
					<?php if( in_array( 'delete_carrier_booking_sea', unserialize($capabilities) ) ){ ?>
...
function nomorbaru()
{
	$.post( 'db_lastid', {tanggal : $( '#order_date' ).val() }, function( result ){
		if( result === '' ){
			...
			$( '#si_no' ).val( pecah[2] + pecah[1] + '0001' );
		} else {
...
function add_row_container(i, action) {
	...
	tabel  += '<td><input type="text" name="container[' + i + '][tipe]" style="text-align:right;width:100px" id="container-' + i + '-tipe" class="container-tipe detail_transaksi" /></td>';
	...
	tabel  += '<td><input type="text" name="container[' + i + '][description]" style="text-align:right;width:100px" id="container-' + i + '-description" class="container-description detail_transaksi currency" /></td>';
	if( action === 1 ){
...
$( document ).ready( function() {
	...
	$( '#carrier' ).bind( 'keydown', function( e ) {
		if ( e.which === 13 ){
			e.preventDefault();
	...
	$( '#carrier' ).bind( 'blur', function( ) {
		if ($( this ).val()===''){
			$( '#carrier_name' ).val( "" );
		}else{
			if ($(this).val()!== $(this).attr("oldvalue")){
				$.post( 'db_read_customer', { id: $( '#carrier' ).val(), cond:''  }, function( result ){
					if( result === '' ){
	...
	$( '.container-size' ).live( 'keydown', function( e ){
		...
		if ( e.which === 13 ){
			e.preventDefault;
			setfocus(Number(num),'tipe','container');
		}else if ( e.which === 40 ){
			e.preventDefault;
	...
	$( '.container-tipe' ).live( 'keydown', function( e ){
		...
		if ( e.which === 13 ){
			...
			e.preventDefault;
			setfocus(Number(num)+1,'tipe','container');
		}else if ( e.which === 38 ){
			e.preventDefault;
			setfocus2(Number(num),'tipe','container');
		}else if ( e.which === 39 ){
			var textBox = document.getElementById($( this ).attr( 'id' ));
			var textLength = textBox.value.length;			
			if (($(this).val()==='') || (getselectedtext($( this ).attr( 'id' ))==textBox.value) || ($(this).caret().start===textBox.value.length)){
	...
	//
	////////////////////////////////////////////////////////////////
	$( '#si_no' ).bind( 'blur', function() {
		if ($(this).val()!==$(this).attr('oldvalue')){
			$.post( 'db_read', { id: $( '#si_no' ).val() }, function( result ){
				if( result === '' ){
					...
					$( "#carrier" ).val( data[ 'carrier_code' ] );
					$( "#carrier_name" ).val( data[ 'carrier_description' ] );
					...
					if( data['container']!=null)
					{
						...
						for(; data['container'][i]; i++ ){
							...
							$( '#container-'+i+'-tipe' ).val( data['container'][i][ 'type' ] );
					...
					$('#cetak').attr('href', 'pdf?si_no=' + $('#si_no').val());
					$( '.simpan' ).hide();
				}
			});
		}
	});
	$( '#si_no' ).bind( 'keydown', function( e ) {
		if ( e.which === 13 ){
			e.preventDefault();
			if( $( this ).val() === '' ){	
				nomorbaru();
		...
		if( $( '#si_no' ).val() !== '' ){
			$.post( 'db_create', $( 'form' ).serialize(), function( result ){
				...
				if( success['status'] ==='Error' ){
					var message = '<div class="alert alert-error" data-dismiss="alert"><a href="#" class="close">&times;</a><strong>Peringatan!</strong> Data '+$( '#si_no' ).val()+' gagal disimpan. '+ success['ket'] +'</div>';
					$('#alert').html( message );					
					if (success['ket']=='Data dengan nomor order yang sama sudah ada.'){
						nomorbaru();
					}
				}else if( success['status'] ){
					var message = '<div class="alert alert-success" data-dismiss="alert"><a href="#" class="close">&times;</a><strong>Berhasil!</strong> Data '+$( '#si_no' ).val()+' telah disimpan.</div>';
					$('#alert').html( message );
					initializex();
				} else {
					var message = '<div class="alert alert-error" data-dismiss="alert"><a href="#" class="close">&times;</a><strong>Error!</strong> Data '+$( '#si_no' ).val()+' gagal disimpan.</div>';
					$('#alert').html( message );
				}
			});
		} else {
	...
	$( '.ubah' ).click( function(){
		if( $( '#si_no' ).val() !== ''  ){
			$.post( 'db_update', $( 'form' ).serialize(), function( result ){
				success=unserialize(result);
				if( success['status'] ==='Error' ){
					var message = '<div class="alert alert-error" data-dismiss="alert"><a href="#" class="close">&times;</a><strong>Peringatan!</strong> Data '+$( '#si_no' ).val()+'  gagal diubah. '+ success['ket'] +'</div>';
					$('#alert').html( message );					
					if (success['ket']=='Data dengan nomor transaksi yang sama sudah ada.'){
						nomorbaru();
					}
				}else if( success['status'] ){
					var message = '<div class="alert alert-success" data-dismiss="alert"><a href="#" class="close">&times;</a><strong>Berhasil!</strong> Data '+$( '#si_no' ).val()+' telah diubah.</div>';
					$('#alert').html( message );
					//$( '#cetak' ).attr( 'href', 'pdf?id_jurnal=' + $('#id_jurnal').val()+'&id_transaksi=' + $('#id_transaksi').val());					
					//$( '.cetak' ).click();
					initializex();
				} else {
					var message = '<div class="alert alert-error" data-dismiss="alert"><a href="#" class="close">&times;</a><strong>Error!</strong> Data '+$( '#si_no' ).val()+' gagal diubah.</div>';
					$('#alert').html( message );
				}
			});
		} else {
	...
	$( '.hapus' ).click( function(){
		if( confirm( 'Anda yakin untuk menghapus data ini?' ) ){
			if( $( '#si_no' ).val() !== '' ){
				$.post( 'db_delete', { id: $( '#si_no' ).val() }, function( result ){
					success=unserialize(result);
					if( success['status'] ==='Error' ){
						var message = '<div class="alert alert-error" data-dismiss="alert"><a href="#" class="close">&times;</a><strong>Peringatan!</strong> Data '+$( '#si_no' ).val()+' gagal dihapus. '+ success['ket'] +'</div>';
						$('#alert').html( message );					
					}else if( success['status'] ){
						var message = '<div class="alert alert-success" data-dismiss="alert"><a href="#" class="close">&times;</a><strong>Berhasil!</strong> Data '+$( '#si_no' ).val()+' telah dihapus.</div>';
						$('#alert').html( message );
						initializex();
					} else {
						var message = '<div class="alert alert-error" data-dismiss="alert"><a href="#" class="close">&times;</a><strong>Error!</strong> Data '+$( '#si_no' ).val()+'  gagal dihapus.</div>';
						$('#alert').html( message );
					}
				});
			} else {
