<div class="container">
	<h2 class="judul">Carrier Booking (AIR)</h2>
	<div id="alert"></div>
		...
		<fieldset>
			<div class="row">
				<div class="control-group span6" style="height:15px">
					...
					<div class="controls">
						<div class="input-append"><input type="text" name="si_no" id="si_no" oldvalue="" browseobj="cari_si_no" /><a style="display:none" class="add-on browse" id="cari_si_no" href="cari/ref/si_no/tipe/carrier_booking_air" title="Click For Search SI No."><i class="icon-search"></i></a></div>
					</div>
				</div>
				<div class="control-group span6" style="height:15px">
			...
			<div class="row">
				<div class="control-group span6" style="height:15px">
					...
					<div class="controls">
						<input type="text" obl="" name="reference" id="reference" />
					</div>
				</div>
			</div>
			<div class="row">
...
<!--							<li><a href="#biaya" class="tab" title="Klik untuk mengisi informasi biaya lain">B. Lain</a></li>-->
			...
			<div class="span12 form-actions btn-group">
				<?php if( in_array( 'create_carrier_booking_air', unserialize($capabilities) ) ){ ?>
				...
				<?php if( in_array( 'update_carrier_booking_air', unserialize($capabilities) ) ){ ?>
				<a href="#" class="btn btn-primary ubah" style="top:-15px"><i class="icon-pencil icon-white"></i> Ubah</a>
				<?php } ?>
				<?php if( intval($role)!= 1){ ?>
					<?php if( in_array( 'delete_carrier_booking_air', unserialize($capabilities) ) ){ ?>
...
preventempty="si_no,order_date,reference";
...
$( document ).ready( function() {
	...
	$( '#reference' ).bind( 'keydown', function( e ) {
		if ( e.which === 13 ){
			e.preventDefault();
			if( $( this ).val() === '' ){
		...
		if( $( '#si_no' ).val() !== '' ){
			$.post( 'db_create', $( 'form' ).serialize(), function( result ){
				...
				if( success['status'] ==='Error' ){
					var message = '<div class="alert alert-error" data-dismiss="alert"><a href="#" class="close">&times;</a><strong>Warning!</strong> Process '+$( '#si_no' ).val()+' Failed. '+ success['ket'] +'</div>';
					$('#alert').html( message );					
					if (success['ket']=='Data Already Exist.'){
						nomorbaru();
					}
				}else if( success['status'] ){
					var message = '<div class="alert alert-success" data-dismiss="alert"><a href="#" class="close">&times;</a><strong>Success!</strong> Process '+$( '#si_no' ).val()+' Complete.</div>';
					$('#alert').html( message );
					initializex();
				} else {
					var message = '<div class="alert alert-error" data-dismiss="alert"><a href="#" class="close">&times;</a><strong>Error!</strong> Process '+$( '#si_no' ).val()+' Failed.</div>';
					$('#alert').html( message );
				}
			});
		} else {
