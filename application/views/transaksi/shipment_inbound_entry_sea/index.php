<div class="container">
	<h2 class="judul">Shipment Inbound Entry (SEA)</h2>
	<div id="alert"></div>
		...
		<fieldset>
			<div class="row">
				<div class="control-group span6" style="height:15px">
					...
					<div class="controls">
						<div class="input-append"><input type="text" name="order_no" id="order_no" oldvalue="" browseobj="cari_order_no" /><a style="display:none" class="add-on browse" id="cari_order_no" href="cari/ref/order_no/tipe/shipment_inbound_entry_sea" title="Klik untuk mencari order number"><i class="icon-search"></i></a></div>
					</div>
				</div>
				<div class="control-group span6" style="height:15px">
...
<!--							<li><a href="#biaya" class="tab" title="Klik untuk mengisi informasi biaya lain">B. Lain</a></li>-->
				...
				<div class="span12 tab-pane shipment" style="height:300px;width:1300px" id="shipment">
					...
					<div class="row">
						<div class="control-group span12" >
							...
							<table class="table">
								<thead height="5px">
									<tr height="5px"><th style="width:100px">Container Code</th><th style="width:100px">Size</th><th style="width:100px">Type</th><th style="width:100px">Seal No.</th><th style="width:100px">Gross Weight</th><th style="width:100px">Nett Weight</th><th style="width:100px">Measurement</th><th></th></tr>
								</thead>
			...
			<div class="span12 form-actions btn-group">
				<?php if( in_array( 'create_shipment_inbound_entry_sea', unserialize($capabilities) ) ){ ?>
				...
				<?php if( in_array( 'update_shipment_inbound_entry_sea', unserialize($capabilities) ) ){ ?>
				<a href="#" class="btn btn-primary ubah" style="top:-15px"><i class="icon-pencil icon-white"></i> Ubah</a>
				<?php } ?>
				<?php if( intval($role)!= 1){ ?>
					<?php if( in_array( 'delete_shipment_inbound_entry_sea', unserialize($capabilities) ) ){ ?>
...
preventempty="order_no,order_date,HBL_no,OBL_no,quotation_ref";
preventemptystatus="1,1,1,1,1";
//*****************************************************************//
//
...
$( document ).ready( function() {
	...
	$( '#shipping_line' ).bind( 'blur', function( ) {
		if ($( this ).val()===''){
			$( '#shipping_line_name' ).val( "" );
		}else{
			if ($(this).val()!== $(this).attr("oldvalue")){
				$.post( 'db_read_customer', { id: $( '#shipping_line' ).val(), cond:'shippingline'  }, function( result ){
					if( result === '' ){
						...
						var data = unserialize( result );	
						$( "#shipping_line_name" ).val( data['name'] );
					}
				});
			}
		}
	});
	$( '#shipping_line' ).bind( 'keydown', function( e ) {
		if ( e.which === 13 ){
			e.preventDefault();
			if( $( this ).val() === '' ){
				$(this).attr('oldvalue',$(this).val());
				$( '#cari_shipping_line' ).click();
			} else {
				$( '#co_loader' ).focus();
			}
		}
		...
		if ($(this).val()!==$(this).attr('oldvalue')){
			$.post( 'db_read', { id: $( '#order_no' ).val() }, function( result ){
				if( result === '' ){
					...
					$( "#shipping_line" ).val( data[ 'shipping_line_code' ] );
					$( "#shipping_line_name" ).val( data[ 'shipping_line_description' ] );
		...
		if( $( '#order_no' ).val() !== '' ){
			$.post( 'db_create', $( 'form' ).serialize(), function( result ){
				...
				if( success['status'] ==='Error' ){
					var message = '<div class="alert alert-error" data-dismiss="alert"><a href="#" class="close">&times;</a><strong>Peringatan!</strong> Data '+$( '#order_no' ).val()+' gagal disimpan. '+ success['ket'] +'</div>';
					$('#alert').html( message );					
					if (success['ket']=='Data dengan nomor order yang sama sudah ada.'){
						nomorbaru();
					}
				}else if( success['status'] ){
					var message = '<div class="alert alert-success" data-dismiss="alert"><a href="#" class="close">&times;</a><strong>Berhasil!</strong> Data '+$( '#order_no' ).val()+' telah disimpan.</div>';
					$('#alert').html( message );
					initializex();
				} else {
					var message = '<div class="alert alert-error" data-dismiss="alert"><a href="#" class="close">&times;</a><strong>Error!</strong> Data '+$( '#order_no' ).val()+' gagal disimpan.</div>';
					$('#alert').html( message );
				}
			});
		} else {
	...
	$( '.ubah' ).click( function(){
		if( $( '#order_no' ).val() !== ''  ){
			$.post( 'db_update', $( 'form' ).serialize(), function( result ){
				success=unserialize(result);
				if( success['status'] ==='Error' ){
					var message = '<div class="alert alert-error" data-dismiss="alert"><a href="#" class="close">&times;</a><strong>Peringatan!</strong> Data '+$( '#order_no' ).val()+'  gagal diubah. '+ success['ket'] +'</div>';
					$('#alert').html( message );					
					if (success['ket']=='Data dengan nomor transaksi yang sama sudah ada.'){
						nomorbaru();
					}
				}else if( success['status'] ){
					var message = '<div class="alert alert-success" data-dismiss="alert"><a href="#" class="close">&times;</a><strong>Berhasil!</strong> Data '+$( '#order_no' ).val()+' telah diubah.</div>';
					$('#alert').html( message );
					//$( '#cetak' ).attr( 'href', 'pdf?id_jurnal=' + $('#id_jurnal').val()+'&id_transaksi=' + $('#id_transaksi').val());					
					//$( '.cetak' ).click();
					initializex();
				} else {
					var message = '<div class="alert alert-error" data-dismiss="alert"><a href="#" class="close">&times;</a><strong>Error!</strong> Data '+$( '#order_no' ).val()+' gagal diubah.</div>';
					$('#alert').html( message );
				}
			});
		} else {
		...
		if( confirm( 'Anda yakin untuk menghapus data ini?' ) ){
			if( $( '#order_no' ).val() !== '' ){
				$.post( 'db_delete', { id: $( '#order_no' ).val() }, function( result ){
					success=unserialize(result);
					if( success['status'] ==='Error' ){
						var message = '<div class="alert alert-error" data-dismiss="alert"><a href="#" class="close">&times;</a><strong>Peringatan!</strong> Data '+$( '#order_no' ).val()+' gagal dihapus. '+ success['ket'] +'</div>';
						$('#alert').html( message );					
					}else if( success['status'] ){
						var message = '<div class="alert alert-success" data-dismiss="alert"><a href="#" class="close">&times;</a><strong>Berhasil!</strong> Data '+$( '#order_no' ).val()+' telah dihapus.</div>';
						$('#alert').html( message );
						initializex();
					} else {
						var message = '<div class="alert alert-error" data-dismiss="alert"><a href="#" class="close">&times;</a><strong>Error!</strong> Data '+$( '#order_no' ).val()+'  gagal dihapus.</div>';
						$('#alert').html( message );
					}
				});
			} else {
