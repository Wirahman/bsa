<div class="container">
	<h2 class="judul">Import Master (AIR)</h2>
	<div id="alert"></div>
		...
		<fieldset>
			<div class="row">
				<div class="control-group span6" style="height:15px">
					...
					<div class="controls">
						<div class="input-append"><input type="text" name="MAWB_no" id="MAWB_no" oldvalue="" browseobj="cari_MAWB_no" /><a style="display:none" class="add-on browse" id="cari_MAWB_no" href="cari/ref/MAWB_no/tipe/air_import_master" title="Click For Search HAWB No."><i class="icon-search"></i></a></div>
					</div>
				</div>
				<div class="control-group span6" style="height:15px">
			...
			<div class="row">
				<div class="control-group span6" style="height:15px">
					...
					<div class="controls">
						<input type="text" obl="" name="total_shipment" id="total_shipment" />
					</div>
				</div>
			</div>
			<div class="row">
...
<!--							<li><a href="#biaya" class="tab" title="Klik untuk mengisi informasi biaya lain">B. Lain</a></li>-->
			...
			<div class="span12 form-actions btn-group">
				<?php if( in_array( 'create_air_import_master', unserialize($capabilities) ) ){ ?>
				...
				<?php if( in_array( 'update_air_import_master', unserialize($capabilities) ) ){ ?>
				<a href="#" class="btn btn-primary ubah" style="top:-15px"><i class="icon-pencil icon-white"></i> Ubah</a>
				<?php } ?>
				<?php if( intval($role)!= 1){ ?>
					<?php if( in_array( 'delete_air_import_master', unserialize($capabilities) ) ){ ?>
...
preventempty="MAWB_no,order_date,total_shipment";
...
function nomorbaru()
{
	$.post( 'db_lastid', {tanggal : $( '#order_date' ).val() }, function( result ){
		if( result === '' ){
			...
			$( '#MAWB_no' ).val( pecah[2] + pecah[1] + '0001' );
		} else {
...
$( document ).ready( function() {
	...
	$( '#MAWB_no' ).bind( 'blur', function() {
		if ($(this).val()!==$(this).attr('oldvalue')){
			$.post( 'db_read', { id: $( '#MAWB_no' ).val() }, function( result ){
				if( result === '' ){
					...
					$('#cetak').attr('href', 'pdf?MAWB_no=' + $('#MAWB_no').val());
					$( '.simpan' ).hide();
				}
			});
		}
		...
		if( $( '#MAWB_no' ).val() !== '' ){
			$.post( 'db_create', $( 'form' ).serialize(), function( result ){
				...
				if( success['status'] ==='Error' ){
					var message = '<div class="alert alert-error" data-dismiss="alert"><a href="#" class="close">&times;</a><strong>Warning!</strong> Process '+$( '#MAWB_no' ).val()+' Failed. '+ success['ket'] +'</div>';
					$('#alert').html( message );					
					if (success['ket']=='Data Already Exist.'){
						nomorbaru();
					}
				}else if( success['status'] ){
					var message = '<div class="alert alert-success" data-dismiss="alert"><a href="#" class="close">&times;</a><strong>Success!</strong> Process '+$( '#MAWB_no' ).val()+' Complete.</div>';
					$('#alert').html( message );
					initializex();
				} else {
					var message = '<div class="alert alert-error" data-dismiss="alert"><a href="#" class="close">&times;</a><strong>Error!</strong> Process '+$( '#MAWB_no' ).val()+' Failed.</div>';
					$('#alert').html( message );
				}
			});
		} else {
	...
	$( '.hapus' ).click( function(){
		if( confirm( 'Are You Sure Want Delete This Data?' ) ){
			if( $( '#MAWB_no' ).val() !== '' ){
				$.post( 'db_delete', { id: $( '#MAWB_no' ).val() }, function( result ){
					success=unserialize(result);
					if( success['status'] ==='Error' ){
						var message = '<div class="alert alert-error" data-dismiss="alert"><a href="#" class="close">&times;</a><strong>Warning!</strong> Process '+$( '#MAWB_no' ).val()+' Failed. '+ success['ket'] +'</div>';
						$('#alert').html( message );					
