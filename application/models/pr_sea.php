...
class Pr_sea extends CI_Model {
	...
	{
	$sql="select p.*,c.description as charges_description from gl_pr_sea_master as p left join gl_charges as c on p.charges_code=c.charges_code where p.id='".$key."'";
	...
	{
	$sql="select p.*,c.port_name as port_awal from gl_pr_sea_master as p left join gl_seaport as c on p.code_awal=c.port_kode where p.id='".$key."'";
		$query = $this->db->query($sql,array());
		if( $query->num_rows > 0 ){
			return $query->row_array();
		} else {
	...
	{
	$sql="select p.*,c.port_name as port_akhir from gl_pr_sea_master as p left join gl_seaport as c on p.code_akhir=c.port_kode where p.id='".$key."'";
		$query = $this->db->query($sql,array());
		if( $query->num_rows > 0 ){
			return $query->row_array();
		} else {
	...
	{
	$sql="select p.*,c.description as unit_description from gl_pr_sea_master as p left join gl_seaport as c on p.unit_code=c.unit_code where p.id='".$key."'";
		$query = $this->db->query($sql,array());
		if( $query->num_rows > 0 ){
			return $query->row_array();
		} else {
	...
	{
	$sql="select p.*,c.description as currency_description from gl_pr_sea_master as p left join gl_seaport as c on p.currency_code=c.currency_code where p.id='".$key."'";
		$query = $this->db->query($sql,array());
		if( $query->num_rows > 0 ){
			return $query->row_array();
		} else {
...
/* End of file pr_sea.php */
/* Location: ./application/models/pr_sea.php */
