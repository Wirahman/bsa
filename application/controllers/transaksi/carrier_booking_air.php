...
class Carrier_booking_air extends CI_Controller{
    private $controller = 'transaksi/carrier_booking_air';

	function __construct()
	{
		parent::__construct();
		$this->load->model( 'Tcarrier_booking_air', 'tcarrier_booking_air');
		$this->load->model( 'Tcarrier_booking_air_route', 'tcarrier_booking_air_route');
	...
	public function index()
	{
		if (!$this->tank_auth->is_logged_in()) {
			...
			if( in_array( 'read_carrier_booking_air', unserialize( $this->tank_auth->get_capabilities()))){
				$this->xlog->record( 'open', 'Carrier_booking_air', '');
				$this->load->view('transaksi/carrier_booking_air/index', $data);
			}else{
...
>>>>>>> 7a6ab39cfd5560f13973f69dc48fa0dada02ad42
	...
	public function db_carrier_booking_air()
	{
		if ($this->tank_auth->is_logged_in()) {
			if( isset( $_POST[ 'key' ])){
				$this->load->model( 'Search', 'search' );
				$hasil = $this->search->search_carrier_booking_air( $_POST['key']);
				echo serialize( $hasil );
			}
		}
	}
	...
	public function db_lastid()
	{
		if ($this->tank_auth->is_logged_in()) {
			if( isset( $_POST[ 'tanggal' ] ) ){
				echo $this->tcarrier_booking_air->lastid( $_POST['tanggal'] );
			}
		}
	}
	public function db_create()
	{
		...
		$resulttrn['ket']='';
		if( in_array( 'create_carrier_booking_air', unserialize( $this->tank_auth->get_capabilities() ) )){
			...
			if ($this->tank_auth->is_logged_in()) {
				if( isset( $_POST['si_no'] )
					...
					if (empty($_POST['si_no']) ){
						...
						for( $i = 0; $i < $_POST['baris-route' ]; $i++){
							if( isset( $_POST[ 'route' ][ $i ][ 'routing' ] )
								...
								$carrier_booking_route[] = $grid_route;
							}
						}
					}
					...
					if ($resulttrn['status']=='Error'){
						$this->xlog->record( 'create', 'carrier_booking_air', $resulttrn);
					...
					$hasil=$this->tcarrier_booking_air->create ( $data );
					if ($hasil=='ada'){
						$resulttrn['status']='Error';
						$resulttrn['ket']='Data Already exist.';
					}elseif ($hasil=='1'){
						$hasil=true;
						$this->tcarrier_booking_air_route->clean( $_POST['si_no']);
						if (isset($carrier_booking_route) && !empty($carrier_booking_route)){
							if (!$this->tcarrier_booking_air_route->create( $carrier_booking_route)){
								$hasil=false;
							}
						}
						if ($hasil){
							...
							$this->tcarrier_booking_air->delete( $_POST['si_no']);
							$this->tcarrier_booking_air_route->clean( $_POST['si_no']);
						}
					}else{
						$resulttrn['status']='Error';
		...
		$this->xlog->record( 'create', 'carrier_booking_air', $resulttrnx);
	...
	public function db_read()
	{
		if ($this->tank_auth->is_logged_in()) {
			if( isset( $_POST['id'] )
				...
				$carrier_booking = $this->tcarrier_booking_air->read( $_POST['id']);
				if( ! empty( $carrier_booking )){
					$carrier_booking['route'] = $this->tcarrier_booking_air_route->load( $_POST['id']);
	...
	public function db_update()
	{
		$resulttrn['status']='Berhasil';
		$resulttrn['ket']='';
		if( in_array( 'update_carrier_booking_air', unserialize( $this->tank_auth->get_capabilities() ) )){
			$this->load->library( 'Emkl', 'emkl' );
			if ($this->tank_auth->is_logged_in()) {
				if( isset( $_POST['si_no'] )
					&& isset( $_POST['order_date'] )
					...
					if ($resulttrn['status']=='Error'){
						$this->xlog->record( 'update', 'carrier_booking_air', $resulttrn );
						echo serialize($resulttrn);
						exit;
					}
					$resulttrnx['isi']=$_POST;
					...
					$hasil=$this->tcarrier_booking_air->update( $data );
					if ($hasil=='1'){
						$hasil=true;
						$this->tcarrier_booking_air_route->clean( $_POST['si_no']);
						if (isset($carrier_booking_route) && !empty($carrier_booking_route)){
		...
		$resulttrnx['status']=$resulttrn['status'];
		$resulttrnx['ket']=$resulttrn['ket'];
		$this->xlog->record( 'update', 'carrier_booking_air', $resulttrnx );
		echo serialize($resulttrn);
	}
	...
	public function db_delete()
	{
		$resulttrn['status']='Berhasil';
		$resulttrn['ket']='';
		if( in_array( 'delete_carrier_booking_air', unserialize( $this->tank_auth->get_capabilities() ) )){
			...
			if ($this->tank_auth->is_logged_in()) {
				if( isset( $_POST['id'] )
					...
					if ($this->tcarrier_booking_air->delete( $_POST['id'] )){
						$this->tcarrier_booking_air_route->clean( $_POST['id']);
					}else{
						$resulttrn['status']='Error';
			...
			$resulttrnx['ket']=$resulttrn['ket'];
			$resulttrnx['isi']=$_POST;
			$this->xlog->record( 'delete', 'carrier_booking_air', $resulttrnx );
			echo serialize($resulttrn);
	}
