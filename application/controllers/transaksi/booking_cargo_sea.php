...
class Booking_cargo_sea extends CI_Controller {
    private $controller = 'transaksi/booking_cargo_sea';
	...
	public function index()
	{
		if (!$this->tank_auth->is_logged_in()) {
			...
			if( in_array( 'read_booking_cargo_sea', unserialize( $this->tank_auth->get_capabilities() ) )){
				$this->xlog->record( 'open', 'Booking_cargo_sea', '' );
				$this->load->view('transaksi/booking_cargo_sea/index', $data);
			}else{
	...
	public function db_booking_cargo_sea()
	{
		if ($this->tank_auth->is_logged_in()) {
			if( isset( $_POST[ 'key' ] ) ){
				$this->load->model( 'Search', 'search' );
				$mcustomer = $this->search->search_booking_cargo_sea( $_POST['key']);
				echo serialize( $mcustomer );
			}
		}
	}
	...
	public function db_lastid()
	{
		if ($this->tank_auth->is_logged_in()) {
			if( isset( $_POST[ 'tanggal' ] ) ){
				echo $this->tbooking_cargo_sea->lastid( $_POST['tanggal'] );
			}
		}
	}
	...
	public function db_create()
	{
		...
		$resulttrn['ket']='';
		if( in_array( 'create_booking_cargo_sea', unserialize( $this->tank_auth->get_capabilities() ) )){
			...
			if ($this->tank_auth->is_logged_in()) {
				if( isset( $_POST['order_no'] )
					...
					if ($resulttrn['status']=='Error'){
						$this->xlog->record( 'create', 'booking_cargo_sea', $resulttrn );
					...
					$hasil=$this->tbooking_cargo_sea->create( $data );
					if ($hasil=='ada'){
						$resulttrn['status']='Error';
						...
						if (isset($booking_cargo_vessel) && !empty($booking_cargo_vessel)){
							if (!$this->tbooking_cargo_sea_vessel->create( $booking_cargo_vessel )){
								$hasil=false;
							}
						}
						if (isset($booking_cargo_container) && !empty($booking_cargo_container)){
							if (!$this->tbooking_cargo_sea_container->create( $booking_cargo_container )){
								$hasil=false;
							}
						}
						if ($hasil){
		...
		$this->xlog->record( 'create', 'booking_cargo_sea', $resulttrnx );
	...
	public function db_read()
	{
		if ($this->tank_auth->is_logged_in()) {
			if( isset( $_POST['id'] )
				...
				$booking_cargo = $this->tbooking_cargo_sea->read( $_POST['id'] );
				if( ! empty( $booking_cargo ) ){
					$booking_cargo['vessel'] = $this->tbooking_cargo_sea_vessel->load( $_POST['id']);
					$booking_cargo['container'] = $this->tbooking_cargo_sea_container->load( $_POST['id']);
	...
	public function db_update()
	{
		$resulttrn['status']='Berhasil';
		$resulttrn['ket']='';
		if( in_array( 'update_booking_cargo_sea', unserialize( $this->tank_auth->get_capabilities() ) )){
			$this->load->library( 'Emkl', 'emkl' );
			if ($this->tank_auth->is_logged_in()) {
				if( isset( $_POST['order_no'] )
					&& isset( $_POST['order_date'] )
					...
					if ($resulttrn['status']=='Error'){
						$this->xlog->record( 'update', 'booking_cargo_sea', $resulttrn );
						echo serialize($resulttrn);
						exit;
					}
					$resulttrnx['isi']=$_POST;
					...
					$hasil=$this->tbooking_cargo_sea->update( $data );
					if ($hasil=='1'){
						$hasil=true;
						$this->tbooking_cargo_sea_vessel->clean( $_POST['order_no']);
						$this->tbooking_cargo_sea_container->clean( $_POST['order_no']);
		...
		$resulttrnx['status']=$resulttrn['status'];
		$resulttrnx['ket']=$resulttrn['ket'];
		$this->xlog->record( 'update', 'booking_cargo_sea', $resulttrnx );
		echo serialize($resulttrn);
	}
	...
	public function db_delete()
	{
		$resulttrn['status']='Berhasil';
		$resulttrn['ket']='';
		if( in_array( 'delete_booking_cargo_sea', unserialize( $this->tank_auth->get_capabilities() ) )){
			...
			if ($this->tank_auth->is_logged_in()) {
				if( isset( $_POST['id'] )
					...
					if ($this->tbooking_cargo_sea->delete( $_POST['id'] )){
						$this->tbooking_cargo_sea_vessel->clean( $_POST['id']);
						$this->tbooking_cargo_sea_container->clean( $_POST['id']);
					}else{
						$resulttrn['status']='Error';
			...
			$resulttrnx['ket']=$resulttrn['ket'];
			$resulttrnx['isi']=$_POST;
			$this->xlog->record( 'delete', 'booking_cargo_sea', $resulttrnx );
			echo serialize($resulttrn);
	}
