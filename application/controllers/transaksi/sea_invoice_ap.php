...
class Sea_invoice_ap extends CI_Controller {
    private $controller = 'transaksi/sea_invoice_ap';
	...
	function __construct()
	{
		...
		$this->load->model( 'Tsea_invoice_ap', 'tsea_invoice_ap');
		$this->load->model( 'Tsea_invoice_ap_charges', 'tsea_invoice_ap_charges');
	...
	public function index()
	{
		if (!$this->tank_auth->is_logged_in()){
			...
			if( in_array( 'read_sea_invoice_ap', unserialize( $this->tank_auth->get_capabilities()))){
				$this->xlog->record( 'open', 'Sea_invoice_ap', '');
				$this->load->view('transaksi/sea_invoice_ap/index', $data);
			}else{
	...
	public function db_sea_invoice_ap()
	{
		if ($this->tank_auth->is_logged_in()) {
			if( isset( $_POST[ 'key' ])){
				$this->load->model( 'Search', 'search' );
				$hasil = $this->search->search_sea_invoice_ap( $_POST['key']);
				echo serialize( $hasil );
			}
		}
	}
	...
	public function db_lastid()
	{
		if ($this->tank_auth->is_logged_in()) {
			if( isset( $_POST[ 'tanggal' ])){
				echo $this->tsea_invoice_ap->lastid( $_POST['tanggal']);
			}
		}
	}
	...
	public function db_create()
	{
		...
		$resulttrn['ket']='';
		if( in_array( 'create_sea_invoice_ap', unserialize( $this->tank_auth->get_capabilities() ) )){
			...
			if ($this->tank_auth->is_logged_in()) {
				if( isset( $_POST['invoice_no'] )
					...
					if ($resulttrn['status']=='Error'){
						$this->xlog->record( 'create', 'sea_invoice_ap', $resulttrn );
					...
					$hasil=$this->tsea_invoice_ap->create( $data );
					if ($hasil=='ada'){
						$resulttrn['status']='Error';
						$resulttrn['ket']='Data Already Exist.';
					}elseif ($hasil=='1'){
						$hasil=true;
						$this->tsea_invoice_ap_charges->clean( $_POST['invoice_no']);
						if (isset($sea_invoice_charges) && !empty($sea_invoice_charges)){
							if (!$this->tsea_invoice_ap_charges->create( $sea_invoice_charges )){
								$hasil=false;
							}
						}
						if ($hasil){
							...
							$this->tsea_invoice_ap->delete( $_POST['invoice_no']);
							$this->tsea_invoice_ap_charges->clean( $_POST['invoice_no'] );
						}
					}else{
						$resulttrn['status']='Error';
		...
		$this->xlog->record( 'create', 'sea_invoice_ap', $resulttrnx );
	...
	public function db_read()
	{
		if ($this->tank_auth->is_logged_in()) {
			if( isset( $_POST['id'])
				...
				$sea_invoice_ap = $this->tsea_invoice_ap->read( $_POST['id']);
				if( ! empty( $sea_invoice_ap )){
					$sea_invoice_ap['charges'] = $this->tsea_invoice_ap_charges->load( $_POST['id']);
					echo serialize( $sea_invoice_ap );
				}
			}
		}
	}
	...
	public function db_update()
	{
		$resulttrn['status']='Berhasil';
		$resulttrn['ket']='';
		if( in_array( 'update_sea_invoice_ap', unserialize( $this->tank_auth->get_capabilities() ) )){
			$this->load->library( 'Emkl', 'emkl' );
			if ($this->tank_auth->is_logged_in()) {
				if( isset( $_POST['invoice_no'] )
					&& isset( $_POST['invoice_date'] )
					...
					if ($resulttrn['status']=='Error'){
						$this->xlog->record( 'update', 'sea_invoice_ap', $resulttrn );
						echo serialize($resulttrn);
						exit;
					}
					$resulttrnx['isi']=$_POST;
					...
					$hasil=$this->tsea_invoice_ap->update( $data );
					if ($hasil=='1'){
						$hasil=true;
						$this->tsea_invoice_ap_charges->clean( $_POST['invoice_no']);
						if (isset($sea_invoice_charges) && !empty($sea_invoice_charges)){
		...
		$resulttrnx['status']=$resulttrn['status'];
		$resulttrnx['ket']=$resulttrn['ket'];
		$this->xlog->record( 'update', 'sea_invoice_ap', $resulttrnx );
		echo serialize($resulttrn);
	}
	...
	public function db_delete()
	{
		$resulttrn['status']='Berhasil';
		$resulttrn['ket']='';
		if( in_array( 'delete_sea_invoice_ap', unserialize( $this->tank_auth->get_capabilities()))){
			if ($this->tank_auth->is_logged_in()) {
				if( isset( $_POST['id'])
					&& !empty( $_POST['id'])
				){
					if ($this->tsea_invoice_ap->delete( $_POST['id'])){
						$this->tsea_invoice_ap_charges->clean( $_POST['id']);
					}else{
						$resulttrn['status']='Error';
		...
		$resulttrnx['ket']=$resulttrn['ket'];
		$resulttrnx['isi']=$_POST;
		$this->xlog->record( 'delete', 'sea_invoice_ap', $resulttrnx);
		echo serialize($resulttrn);
	}
...
/* Location: ./application/controllers/sea_invoice_ap.php */
