...
class Air_export_master extends CI_Controller{
    private $controller = 'transaksi/air_export_master';

	function __construct()
	{
		parent::__construct();
		$this->load->model( 'Tair_export_master', 'tair_export_master');
		$this->load->model( 'Tair_export_master_route', 'tair_export_master_route');
	...
	public function index()
	{
		if (!$this->tank_auth->is_logged_in()) {
			...
			if( in_array( 'read_air_export_master', unserialize( $this->tank_auth->get_capabilities()))){
				$this->xlog->record( 'open', 'Air_export_master', '');
				$this->load->view('transaksi/air_export_master/index', $data);
			}else{
	...
	public function db_air_export_master()
	{
		if ($this->tank_auth->is_logged_in()) {
			if( isset( $_POST[ 'key' ])){
				$this->load->model( 'Search', 'search' );
				$hasil = $this->search->search_air_export_master( $_POST['key']);
				echo serialize( $hasil );
			}
		}
	}
	...
	public function db_lastid()
	{
		if ($this->tank_auth->is_logged_in()) {
			if( isset( $_POST[ 'tanggal' ] ) ){
				echo $this->tair_export_master->lastid( $_POST['tanggal'] );
			}
		}
	}
	public function db_create()
	{
		...
		$resulttrn['ket']='';
		if( in_array( 'create_air_export_master', unserialize( $this->tank_auth->get_capabilities() ) )){
			...
			if ($this->tank_auth->is_logged_in()) {
				if( isset( $_POST['MAWB_no'] )
					...
					if ($resulttrn['status']=='Error'){
						$this->xlog->record( 'create', 'air_export_master', $resulttrn);
					...
					$hasil=$this->tair_export_master->create ( $data );
					if ($hasil=='ada'){
						$resulttrn['status']='Error';
						$resulttrn['ket']='Data Already exist.';
					}elseif ($hasil=='1'){
						$hasil=true;
						$this->tair_export_master_route->clean( $_POST['MAWB_no']);
						if (isset($air_export_route) && !empty($air_export_route)){
							if (!$this->tair_export_master_route->create( $air_export_route)){
								$hasil=false;
							}
						}
						if ($hasil){
							...
							$this->tair_export_master->delete( $_POST['MAWB_no']);
							$this->tair_export_master_route->clean( $_POST['MAWB_no']);
						}
					}else{
						$resulttrn['status']='Error';
		...
		$this->xlog->record( 'create', 'air_export_master', $resulttrnx);
	...
	public function db_read()
	{
		if ($this->tank_auth->is_logged_in()) {
			if( isset( $_POST['id'] )
				...
				$air_export = $this->tair_export_master->read( $_POST['id']);
				if( ! empty( $air_export )){
					$air_export['route'] = $this->tair_export_master_route->load( $_POST['id']);
					echo serialize( $air_export );
				}
			}
		}
	}
	...
	public function db_update()
	{
		$resulttrn['status']='Berhasil';
		$resulttrn['ket']='';
		if( in_array( 'update_air_export_master', unserialize( $this->tank_auth->get_capabilities() ) )){
			$this->load->library( 'Emkl', 'emkl' );
			if ($this->tank_auth->is_logged_in()) {
				if( isset( $_POST['MAWB_no'] )
					&& isset( $_POST['order_date'] )
					...
					if ($resulttrn['status']=='Error'){
						$this->xlog->record( 'update', 'air_export_master', $resulttrn );
						echo serialize($resulttrn);
						exit;
					}
					$resulttrnx['isi']=$_POST;
					...
					$hasil=$this->tair_export_master->update( $data );
					if ($hasil=='1'){
						$hasil=true;
						$this->tair_export_master_route->clean( $_POST['MAWB_no']);
						if (isset($air_export_route) && !empty($air_export_route)){
		...
		$resulttrnx['status']=$resulttrn['status'];
		$resulttrnx['ket']=$resulttrn['ket'];
		$this->xlog->record( 'update', 'air_export_master', $resulttrnx );
		echo serialize($resulttrn);
	}
	...
	public function db_delete()
	{
		$resulttrn['status']='Berhasil';
		$resulttrn['ket']='';
		if( in_array( 'delete_air_export_master', unserialize( $this->tank_auth->get_capabilities() ) )){
			...
			if ($this->tank_auth->is_logged_in()) {
				if( isset( $_POST['id'] )
					...
					if ($this->tair_export_master->delete( $_POST['id'] )){
						$this->tair_export_master_route->clean( $_POST['id']);
					}else{
						$resulttrn['status']='Error';
			...
			$resulttrnx['ket']=$resulttrn['ket'];
			$resulttrnx['isi']=$_POST;
			$this->xlog->record( 'delete', 'air_export_master', $resulttrnx );
			echo serialize($resulttrn);
	}
