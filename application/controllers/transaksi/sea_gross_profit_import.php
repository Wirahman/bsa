...
class Sea_gross_profit_import extends CI_Controller {

	function __construct()
	{
		parent::__construct();
		$this->load->model( 'Tsea_gross_profit_import', 'tsea_gross_profit_import' );
		$this->load->model( 'Tsea_gross_profit_import_revenue', 'tsea_gross_profit_import_revenue' );
		$this->load->model( 'Tsea_gross_profit_import_cost', 'tsea_gross_profit_import_cost' );
	...
	public function index()
	{
		if (!$this->tank_auth->is_logged_in()) {
			...
			if( in_array( 'read_sea_gross_profit_import', unserialize( $this->tank_auth->get_capabilities() ) )){
				$this->xlog->record( 'open', 'Sea_gross_profit_import', '' );
				$this->load->view('transaksi/sea_gross_profit_import/index', $data);
			}else{
	...
	public function db_read_pr_sea()
	{
		if ($this->tank_auth->is_logged_in()) {
			if( isset( $_POST['id'])
				&& ! empty( $_POST['id'])
			){
				$this->xlog->record( 'read', 'pr_sea', $_POST['id'] );
	...
	public function db_sea_gross_profit_import()
	{
		if ($this->tank_auth->is_logged_in()) {
			if( isset( $_POST[ 'key' ] ) ){
				$this->load->model( 'Search', 'search' );
				$mcustomer = $this->search->search_sea_gross_profit_import( $_POST['key']);
				echo serialize( $mcustomer );
			}
		}
	}
	...
	public function db_lastid()
	{
		if ($this->tank_auth->is_logged_in()) {
			if( isset( $_POST[ 'tanggal' ] ) ){
				echo $this->tsea_gross_profit_import->lastid( $_POST['tanggal'] );
			}
		}
	}
	...
	public function db_create()
	{
		...
		$resulttrn['ket']='';
		if( in_array( 'create_sea_gross_profit_import', unserialize( $this->tank_auth->get_capabilities() ) )){
			...
			if ($this->tank_auth->is_logged_in()) {
				if( isset( $_POST['gp_no'] )
					...
					if (empty($_POST['gp_no']) ){
						...
						for( $i = 0; $i < $_POST[ 'baris-revenue' ]; $i++ ){
							if( isset( $_POST[ 'revenue' ][ $i ][ 'charges_code' ] )
								...
								$sea_gross_revenue[] = $grid_revenue;
							}
						}
						$sea_gross_cost = array();
						$baris=1;
					...
					if ($resulttrn['status']=='Error'){
						$this->xlog->record( 'create', 'sea_gross_profit_import', $resulttrn );
					...
					$hasil=$this->tsea_gross_profit_import->create( $data );
					if ($hasil=='ada'){
						$resulttrn['status']='Error';
						$resulttrn['ket']='Data Already Exist.';
					}elseif ($hasil=='1'){
						$hasil=true;
						$this->tsea_gross_profit_import_revenue->clean( $_POST['gp_no']);
						if (isset($sea_gross_revenue) && !empty($sea_gross_revenue)){
							if (!$this->tsea_gross_profit_import_revenue->create( $sea_gross_revenue )){
								$hasil=false;
							}
						}
						if (isset($sea_gross_cost) && !empty($sea_gross_cost)){
							if (!$this->tsea_gross_profit_import_cost->create( $sea_gross_cost )){
								$hasil=false;
							}
						}
						if ($hasil){
							...
							$this->tsea_gross_profit_import->delete( $_POST['gp_no']);
							$this->tsea_gross_profit_import_revenue->clean( $_POST['gp_no'] );
							$this->tsea_gross_profit_import_cost->clean( $_POST['gp_no'] );
						}
					}else{
						$resulttrn['status']='Error';
		...
		$this->xlog->record( 'create', 'sea_gross_profit_import', $resulttrnx );
	...
	public function db_read()
	{
		if ($this->tank_auth->is_logged_in()) {
			if( isset( $_POST['id'] )
				...
				$sea_gross_profit = $this->tsea_gross_profit_import->read( $_POST['id'] );
				if( ! empty( $sea_gross_profit ) ){
					$sea_gross_profit['revenue'] = $this->tsea_gross_profit_import_revenue->load( $_POST['id']);
					$sea_gross_profit['cost'] = $this->tsea_gross_profit_import_cost->load( $_POST['id']);
					echo serialize( $sea_gross_profit );
				}
			}
		}
	}
	...
	public function db_update()
	{
		$resulttrn['status']='Berhasil';
		$resulttrn['ket']='';
		if( in_array( 'update_sea_gross_profit_import', unserialize( $this->tank_auth->get_capabilities() ) )){
			$this->load->library( 'Emkl', 'emkl' );
			if ($this->tank_auth->is_logged_in()) {
				if( isset( $_POST['gp_no'] )
					&& isset( $_POST['order_date'] )
					...
					if ($resulttrn['status']=='Error'){
						$this->xlog->record( 'update', 'sea_gross_profit_import', $resulttrn );
						echo serialize($resulttrn);
						exit;
					}
					$resulttrnx['isi']=$_POST;
					...
					$hasil=$this->tsea_gross_profit_import->update( $data );
					if ($hasil=='1'){
						$hasil=true;
						$this->tsea_gross_profit_import_revenue->clean( $_POST['gp_no']);
						$this->tsea_gross_profit_import_cost->clean( $_POST['gp_no']);
		...
		$resulttrnx['status']=$resulttrn['status'];
		$resulttrnx['ket']=$resulttrn['ket'];
		$this->xlog->record( 'update', 'sea_gross_profit_import', $resulttrnx );
		echo serialize($resulttrn);
	}
	...
	public function db_delete()
	{
		$resulttrn['status']='Berhasil';
		$resulttrn['ket']='';
		if( in_array( 'delete_sea_gross_profit_import', unserialize( $this->tank_auth->get_capabilities() ) )){
			...
			if ($this->tank_auth->is_logged_in()) {
				if( isset( $_POST['id'] )
					...
					if ($this->tsea_gross_profit_import->delete( $_POST['id'] )){
						$this->tsea_gross_profit_import_revenue->clean( $_POST['id']);
						$this->tsea_gross_profit_import_cost->clean( $_POST['id']);
					}else{
						$resulttrn['status']='Error';
			...
			$resulttrnx['ket']=$resulttrn['ket'];
			$resulttrnx['isi']=$_POST;
			$this->xlog->record( 'delete', 'sea_gross_profit_import', $resulttrnx );
			echo serialize($resulttrn);
	}
