...
function __construct()
{
	...
	$this->load->model( 'Tsea_export_master_vessel', 'tsea_export_master_vessel' );
	$this->load->model( 'Tsea_export_master_container', 'tsea_export_master_container' );
...
public function index()
{
	if(!$this->tank_auth->is_logged_in()){
		...
		if( in_array('read_sea_export_master', unserialize($this->tank_auth->get_capabilities()))){
			$this->xlog->record('open', 'Sea_export_master', '');
			$this->load->view('transaksi/sea_export_master/index', $data);
		}else{
...
public function db_create()
{
	...
	$resulttrn['ket']='';
	if( in_array( 'create_sea_export_master', unserialize($this->tank_auth->get_capabilities()))){
		...
		if ($this->tank_auth->is_logged_in()){
			if( isset( $_POST['OBL_no'])
				...
				if(empty($_POST['OBL_no'])){
					...
					$resulttrn['status']='Error';
					$resulttrn['ket']='Date <strong> MUST </strong> be in format dd-MM-yyyy.';
				}else{
					$sea_export_master_vessel = array();
					$baris=1;
					for( $i = 0; $i < $_POST[ 'baris-vessel']; $i++){
						if( isset( $_POST[ 'vessel' ][ $i ] [ 'code' ])
							...
							if (empty($_POST[ 'vessel' ][ $i ][ 'voyage' ])){
								$resulttrn[ 'status']='Error';
								$resulttrn['ket']='Voyage in line '. $baris .' must be filled';
							}elseif (empty($_POST[ 'vessel' ][ $i ][ 'etd' ]) || !$this->emkl->cek_format_tanggal($_POST[ 'vessel' ][ $i ][ 'etd' ])){
								$resulttrn['status']='Error';
								$resulttrn['ket']='Etd in line '. $baris .' must filled with format dd-MM-yyyy';
							}elseif(empty($_POST[ 'vessel' ][ $i ][ 'eta' ]) || !$this->emkl->cek_format_tanggal($_POST[ 'vessel' ][ $i ][ 'eta' ])){
								$resulttrn['status']='Error';
								$resulttrn['ket']='Eta in line '. $baris .' must filled with format dd-MM-yyyy';
							}else{
								$baris++;
							}
							$grid_vessel = array(
							...
							$sea_export_master_vessel[] = $grid_vessel;
						}
					}
					$sea_export_master_container =array();
					$baris=1;
					for ($i = 0; $i < $_POST[ 'baris-container' ]; $i++) { 
						if ( isset( $_POST[ 'container' ][ $i ][ 'code' ])
							...
							if (empty($_POST[ 'container' ][ $i ][ 'size' ])) {
								$resulttrn['status']='Error';
								$resulttrn['ket']='Size in line '. $baris .' <strong> MUST</strong> filled';
							}elseif (empty($_POST[ 'container' ][ $i ][ 'sealno' ])){
								$resulttrn['status']='Error';
								$resulttrn['ket']='Seal Number in line'. $baris .'<strong> MUST </strong> filled';
								...
								$resulttrn['status']='Error';
								$resulttrn['ket']='Gross Weight in line '. $baris .' <strong> MUST </strong> filled with numeric.';
								...
								$resulttrn['status']='Error';
								$resulttrn['ket']='Net Weight in line '. $baris .' <strong> MUST </strong> filled with numeric.';
								...
								$resulttrn['status']='Error';
								$resulttrn['ket']='Measurement in line '. $baris .'<strong> MUST </strong> filled with numeric.';
							}else{
								$baris++;
							}
							...
							$sea_export_master_container[] = $grid_container;
						}
					}
				...
				if ($resulttrn['status']=='Error'){
					$this->xlog->record( 'create', 'sea_export_master', $resulttrn );
				...
				$hasil=$this->tsea_export_master->create( $data );
				if ($hasil=='ada'){
					$resulttrn['status']='Error';
					$resulttrn['ket']=' Data with same OBL number already Exist.';
				}elseif ($hasil=='1'){
					$hasil=true;
					$this->tsea_export_master_vessel->clean( $_POST['OBL_no']);
					if(isset($sea_export_master_vessel) && !empty($sea_export_master_vessel)){
						if(!$this->tsea_export_master_vessel->create( $sea_export_master_vessel)){
						$hasil=false;
					}
				}
				if(isset($sea_export_master_container)&& !empty($sea_export_master_container)){
					if (!$this->tsea_export_master_container->create( $sea_export_master_container)){
						$hasil=false;
					}
				}
				if ($hasil){
					...
					$this->tsea_export_master->delete( $_POST['OBL_no']);
					$this->tsea_export_master_vessel->clean( $_POST['OBL_no']);
					$this->tsea_export_master_container->clean( $_POST['OBL_no']);
				}
			}else{
				$resulttrn['status']='Error';
...
$this->xlog->record( 'create', 'sea_export_master', $resulttrnx );
...
public function db_read()
	{
		if ($this->tank_auth->is_logged_in()) {
			if( isset( $_POST['id'] )
				...
				$sea_export_master = $this->tsea_export_master->read( $_POST['id'] );
				if( ! empty( $sea_export_master ) ){
					$sea_export_master['vessel'] = $this->tsea_export_master_vessel->load( $_POST['id']);
					$sea_export_master['container'] = $this->tsea_export_master_container->load( $_POST['id']);
					echo serialize( $sea_export_master );
				}
			}
		}
	}
	...
	public function db_update()
	{
		$resulttrn['status']='Berhasil';
		$resulttrn['ket']='';
		if( in_array( 'update_sea_export_master', unserialize( $this->tank_auth->get_capabilities()))){
			$this->load->library( 'Emkl', 'emkl' );
			if ($this->tank_auth->is_logged_in()){
				if ( isset( $_POST['OBL_no'])
					&& isset( $_POST['date'])
				...
				if ($resulttrn['status']=='Error'){
					$this->xlog->record( 'update', 'sea_export_master', $resulttrn);
					echo serialize($resulttrn);
					exit;
				}
				$resulttrnx['isi']=$_POST;
				...
				$hasil=$this->tsea_export_master->update( $data );
				if ($hasil=='1'){
					$hasil=true;
					$this->tsea_export_master_vessel->clean( $_POST['OBL_no']);
					$this->tsea_export_master_container->clean( $_POST['OBL_no']);
	...
	$resulttrnx['status']=$resulttrn['status'];
	$resulttrnx['ket']=$resulttrn['ket'];
	$this->xlog->record( 'update', 'sea_export_master', $resulttrnx);
...
public function db_delete()
{
	$resulttrn['status']='Berhasil';
	$resulttrn['ket']='';
	if( in_array( 'delete_sea_export_master', unserialize( $this->tank_auth->get_capabilities()))){
		if ($this->tank_auth->is_logged_in()){
			if( isset ($_POST['id'])
				&& !empty( $_POST['id'])
				){
				if( $this->tsea_export_master->delete( $_POST['id'])){
					$this->tsea_export_master_vessel->clean( $_POST['id']);
					$this->tsea_export_master_container->clean( $_POST['id']);
				}else{
					$resulttrn['status']='Error';
	...
	$resulttrnx['ket']=$resulttrn['ket'];
	$resulttrnx['isi']=$_POST;
	$this->xlog->record( 'delete', 'sea_export_master', $resulttrnx);
	echo serialize($resulttrn);
}
// Edit Function Kepala dan pdf setelah membuat view.
