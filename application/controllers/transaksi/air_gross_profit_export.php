...
class Air_gross_profit_export extends CI_Controller {

	function __construct()
	{
		parent::__construct();
		$this->load->model( 'Tair_gross_profit_export', 'tair_gross_profit_export' );
		$this->load->model( 'Tair_gross_profit_export_revenue', 'tair_gross_profit_export_revenue' );
		$this->load->model( 'Tair_gross_profit_export_cost', 'tair_gross_profit_export_cost' );
	...
	public function index()
	{
		if (!$this->tank_auth->is_logged_in()) {
			...
			if( in_array( 'read_air_gross_profit_export', unserialize( $this->tank_auth->get_capabilities() ) )){
				$this->xlog->record( 'open', 'Air_gross_profit_export', '' );
				$this->load->view('transaksi/air_gross_profit_export/index', $data);
			}else{
	...
	public function db_air_gross_profit_export()
	{
		if ($this->tank_auth->is_logged_in()) {
			if( isset( $_POST[ 'key' ] ) ){
				$this->load->model( 'Search', 'search' );
				$mcustomer = $this->search->search_air_gross_profit_export( $_POST['key']);
				echo serialize( $mcustomer );
			}
		}
	}
	...
	public function db_lastid()
	{
		if ($this->tank_auth->is_logged_in()) {
			if( isset( $_POST[ 'tanggal' ] ) ){
				echo $this->tair_gross_profit_export->lastid( $_POST['tanggal'] );
			}
		}
	}
	...
	public function db_create()
	{
		...
		$resulttrn['ket']='';
		if( in_array( 'create_air_gross_profit_export', unserialize( $this->tank_auth->get_capabilities() ) )){
			...
			if ($this->tank_auth->is_logged_in()) {
				if( isset( $_POST['gp_no'] )
					...
					if ($resulttrn['status']=='Error'){
						$this->xlog->record( 'create', 'air_gross_profit_export', $resulttrn );
					...
					$hasil=$this->tair_gross_profit_export->create( $data );
					if ($hasil=='ada'){
						$resulttrn['status']='Error';
						$resulttrn['ket']='Data Already Exist.';
					}elseif ($hasil=='1'){
						$hasil=true;
						$this->tair_gross_profit_export_revenue->clean( $_POST['gp_no']);
						if (isset($air_gross_revenue) && !empty($air_gross_revenue)){
							if (!$this->tair_gross_profit_export_revenue->create( $air_gross_revenue )){
								$hasil=false;
							}
						}
						if (isset($air_gross_cost) && !empty($air_gross_cost)){
							if (!$this->tair_gross_profit_export_cost->create( $air_gross_cost )){
								$hasil=false;
							}
						}
						if ($hasil){
							...
							$this->tair_gross_profit_export->delete( $_POST['gp_no']);
							$this->tair_gross_profit_export_revenue->clean( $_POST['gp_no'] );
							$this->tair_gross_profit_export_cost->clean( $_POST['gp_no'] );
						}
					}else{
						$resulttrn['status']='Error';
		...
		$this->xlog->record( 'create', 'air_gross_profit_export', $resulttrnx );
	...
	public function db_read()
	{
		if ($this->tank_auth->is_logged_in()) {
			if( isset( $_POST['id'] )
				...
				$air_gross_profit = $this->tair_gross_profit_export->read( $_POST['id'] );
				if( ! empty( $air_gross_profit ) ){
					$air_gross_profit['revenue'] = $this->tair_gross_profit_export_revenue->load( $_POST['id']);
					$air_gross_profit['cost'] = $this->tair_gross_profit_export_cost->load( $_POST['id']);
	...
	public function db_update()
	{
		$resulttrn['status']='Berhasil';
		$resulttrn['ket']='';
		if( in_array( 'update_air_gross_profit_export', unserialize( $this->tank_auth->get_capabilities() ) )){
			$this->load->library( 'Emkl', 'emkl' );
			if ($this->tank_auth->is_logged_in()) {
				if( isset( $_POST['gp_no'] )
					&& isset( $_POST['order_date'] )
					...
					if ($resulttrn['status']=='Error'){
						$this->xlog->record( 'update', 'air_gross_profit_export', $resulttrn );
						echo serialize($resulttrn);
						exit;
					}
					$resulttrnx['isi']=$_POST;
					...
					$hasil=$this->tair_gross_profit_export->update( $data );
					if ($hasil=='1'){
						$hasil=true;
						$this->tair_gross_profit_export_revenue->clean( $_POST['gp_no']);
						$this->tair_gross_profit_export_cost->clean( $_POST['gp_no']);
		...
		$resulttrnx['status']=$resulttrn['status'];
		$resulttrnx['ket']=$resulttrn['ket'];
		$this->xlog->record( 'update', 'air_gross_profit_export', $resulttrnx );
		echo serialize($resulttrn);
	}
	...
	public function db_delete()
	{
		$resulttrn['status']='Berhasil';
		$resulttrn['ket']='';
		if( in_array( 'delete_air_gross_profit_export', unserialize( $this->tank_auth->get_capabilities() ) )){
			...
			if ($this->tank_auth->is_logged_in()) {
				if( isset( $_POST['id'] )
					...
					if ($this->tair_gross_profit_export->delete( $_POST['id'] )){
						$this->tair_gross_profit_export_revenue->clean( $_POST['id']);
						$this->tair_gross_profit_export_cost->clean( $_POST['id']);
					}else{
						$resulttrn['status']='Error';
			...
			$resulttrnx['ket']=$resulttrn['ket'];
			$resulttrnx['isi']=$_POST;
			$this->xlog->record( 'delete', 'air_gross_profit_export', $resulttrnx );
			echo serialize($resulttrn);
	}
