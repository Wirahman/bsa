...
class Carrier_booking_sea extends CI_Controller {
    private $controller = 'transaksi/carrier_booking_sea';

	function __construct()
	{
		parent::__construct();
		$this->load->model( 'Tcarrier_booking_sea', 'tcarrier_booking_sea' );
		$this->load->model( 'Tcarrier_booking_sea_vessel', 'tcarrier_booking_sea_vessel' );
		$this->load->model( 'Tcarrier_booking_sea_container', 'tcarrier_booking_sea_container' );
	...
	public function index()
	{
		if (!$this->tank_auth->is_logged_in()) {
			...
			if( in_array( 'read_carrier_booking_sea', unserialize( $this->tank_auth->get_capabilities() ) )){
				$this->xlog->record( 'open', 'carrier_booking_sea', '' );
				$this->load->view('transaksi/carrier_booking_sea/index', $data);
			}else{
	...
	public function db_carrier_booking_sea()
	{
		if ($this->tank_auth->is_logged_in()) {
			if( isset( $_POST[ 'key' ] ) ){
				$this->load->model( 'Search', 'search' );
				$mcustomer = $this->search->search_carrier_booking_sea( $_POST['key']);
				echo serialize( $mcustomer );
			}
		}
	}
	...
	public function db_lastid()
	{
		if ($this->tank_auth->is_logged_in()) {
			if( isset( $_POST[ 'tanggal' ] ) ){
				echo $this->tcarrier_booking_sea->lastid( $_POST['tanggal'] );
			}
		}
	}
	...
	public function db_create()
	{
		...
		$resulttrn['ket']='';
		if( in_array( 'create_carrier_booking_sea', unserialize( $this->tank_auth->get_capabilities() ) )){
			$this->load->library( 'Emkl', 'emkl' );
			if ($this->tank_auth->is_logged_in()) {
				if( isset( $_POST['si_no'] )
					&& isset( $_POST['order_date'] )
				){
					$baris=1;
					if (empty($_POST['si_no']) ){
						$resulttrn['status']='Error';
						$resulttrn['ket']='SI Number <strong>MUST</strong> be filled.';
						...
						for( $i = 0; $i < $_POST[ 'baris-vessel' ]; $i++ ){
							if( isset( $_POST[ 'vessel' ][ $i ][ 'code' ] )
								...
								elseif (empty($_POST[ 'vessel' ][ $i ][ 'eta' ]) || !$this->emkl->cek_format_tanggal($_POST[ 'vessel' ][ $i ][ 'eta' ])){
									...
									$carrier_booking_vessel[] = $grid_vessel;
								
							}
						}
						$carrier_booking_container = array();
						$baris=1;
						for( $i = 0; $i < $_POST[ 'baris-container' ]; $i++ ){
							if( isset( $_POST[ 'container' ][ $i ][ 'code' ] )
								...
								$carrier_booking_container[] = $grid_container;
							}
						}
					}
					...
					if ($resulttrn['status']=='Error'){
						$this->xlog->record( 'create', 'carrier_booking_sea', $resulttrn );
						echo serialize($resulttrn);
						exit;
					}
					$resulttrnx['isi']=$_POST;
					$data['si_no'] = $_POST['si_no'];
					...
					$data['carrier_code'] = $_POST['carrier'];
					$data['carrier_description'] = $_POST['carrier_name'];
					...
					$hasil=$this->tcarrier_booking_sea->create( $data );
					if ($hasil=='ada'){
						$resulttrn['status']='Error';
						$resulttrn['ket']='Data dengan order number yang sama sudah ada.';
					}elseif ($hasil=='1'){
						$hasil=true;
						$this->tcarrier_booking_sea_vessel->clean( $_POST['si_no']);
						if (isset($carrier_booking_vessel) && !empty($carrier_booking_vessel)){
							if (!$this->tcarrier_booking_sea_vessel->create( $carrier_booking_vessel )){
								$hasil=false;
							}
						}
						if (isset($carrier_booking_container) && !empty($carrier_booking_container)){
							if (!$this->tcarrier_booking_sea_container->create( $carrier_booking_container )){
								$hasil=false;
							}
						}
						if ($hasil){
							...
							$this->tcarrier_booking_sea->delete( $_POST['si_no']);
							$this->tcarrier_booking_sea_vessel->clean( $_POST['si_no'] );
							$this->tcarrier_booking_sea_container->clean( $_POST['si_no'] );
						}
					}else{
						$resulttrn['status']='Error';
		...
		$this->xlog->record( 'create', 'carrier_booking_sea', $resulttrnx );
	...
	public function db_read()
	{
		if ($this->tank_auth->is_logged_in()) {
			if( isset( $_POST['id'] )
				...
				$carrier_booking = $this->tcarrier_booking_sea->read( $_POST['id'] );
				if( ! empty( $carrier_booking ) ){
					$carrier_booking['vessel'] = $this->tcarrier_booking_sea_vessel->load( $_POST['id']);
					$carrier_booking['container'] = $this->tcarrier_booking_sea_container->load( $_POST['id']);
					echo serialize( $carrier_booking );
				}
			}
		}
	}
	...
	public function db_update()
	{
		$resulttrn['status']='Berhasil';
		$resulttrn['ket']='';
		if( in_array( 'update_carrier_booking_sea', unserialize( $this->tank_auth->get_capabilities() ) )){
			$this->load->library( 'Emkl', 'emkl' );
			if ($this->tank_auth->is_logged_in()) {
				if( isset( $_POST['si_no'] )
					&& isset( $_POST['order_date'] )
					...
					if ($resulttrn['status']=='Error'){
						$this->xlog->record( 'update', 'carrier_booking_sea', $resulttrn );
						echo serialize($resulttrn);
						exit;
					}
					$resulttrnx['isi']=$_POST;
					...
					$hasil=$this->tcarrier_booking_sea->update( $data );
					if ($hasil=='1'){
						$hasil=true;
						$this->tcarrier_booking_sea_vessel->clean( $_POST['si_no']);
						$this->tcarrier_booking_sea_container->clean( $_POST['si_no']);
		...
		$resulttrnx['status']=$resulttrn['status'];
		$resulttrnx['ket']=$resulttrn['ket'];
		$this->xlog->record( 'update', 'carrier_booking_sea', $resulttrnx );
		echo serialize($resulttrn);
	}
	...
	public function db_delete()
	{
		$resulttrn['status']='Berhasil';
		$resulttrn['ket']='';
		if( in_array( 'delete_carrier_booking_sea', unserialize( $this->tank_auth->get_capabilities() ) )){
			...
			if ($this->tank_auth->is_logged_in()) {
				if( isset( $_POST['id'] )
					...
					if ($this->tcarrier_booking_sea->delete( $_POST['id'] )){
						$this->tcarrier_booking_sea_vessel->clean( $_POST['id']);
						$this->tcarrier_booking_sea_container->clean( $_POST['id']);
					}else{
						$resulttrn['status']='Error';
			...
			$resulttrnx['ket']=$resulttrn['ket'];
			$resulttrnx['isi']=$_POST;
			$this->xlog->record( 'delete', 'carrier_booking_sea', $resulttrnx );
			echo serialize($resulttrn);
	}
