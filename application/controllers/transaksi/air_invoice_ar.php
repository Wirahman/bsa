...
class Air_invoice_ar extends CI_Controller {
    private $controller = 'transaksi/air_invoice_ar';
	...
	function __construct()
	{
		...
		$this->load->model( 'Tair_invoice_ar', 'tair_invoice_ar');
		$this->load->model( 'Tair_invoice_ar_charges', 'tair_invoice_ar_charges');
	...
	public function index()
	{
		if (!$this->tank_auth->is_logged_in()){
			...
			if( in_array( 'read_air_invoice_ar', unserialize( $this->tank_auth->get_capabilities()))){
				$this->xlog->record( 'open', 'Air_invoice_ar', '');
				$this->load->view('transaksi/air_invoice_ar/index', $data);
			}else{
	...
	public function db_air_invoice_ar()
	{
		if ($this->tank_auth->is_logged_in()) {
			if( isset( $_POST[ 'key' ])){
				$this->load->model( 'Search', 'search' );
				$hasil = $this->search->search_air_invoice_ar( $_POST['key']);
				echo serialize( $hasil );
			}
		}
	}
	...
	public function db_lastid()
	{
		if ($this->tank_auth->is_logged_in()) {
			if( isset( $_POST[ 'tanggal' ])){
				echo $this->tair_invoice_ar->lastid( $_POST['tanggal']);
			}
		}
	}
	...
	public function db_create()
	{
		...
		$resulttrn['ket']='';
		if( in_array( 'create_air_invoice_ar', unserialize( $this->tank_auth->get_capabilities() ) )){
			...
			if ($this->tank_auth->is_logged_in()) {
				if( isset( $_POST['invoice_no'] )
					...
					if ($resulttrn['status']=='Error'){
						$this->xlog->record( 'create', 'air_invoice_ar', $resulttrn );
					...
					$hasil=$this->tair_invoice_ar->create( $data );
					if ($hasil=='ada'){
						$resulttrn['status']='Error';
						$resulttrn['ket']='Data Already Exist.';
					}elseif ($hasil=='1'){
						$hasil=true;
						$this->tair_invoice_ar_charges->clean( $_POST['invoice_no']);
						if (isset($air_invoice_charges) && !empty($air_invoice_charges)){
							if (!$this->tair_invoice_ar_charges->create( $air_invoice_charges )){
								$hasil=false;
							}
						}
						if ($hasil){
							...
							$this->tair_invoice_ar->delete( $_POST['invoice_no']);
							$this->tair_invoice_ar_charges->clean( $_POST['invoice_no'] );
						}
					}else{
						$resulttrn['status']='Error';
		...
		$this->xlog->record( 'create', 'air_invoice_ar', $resulttrnx );
	...
	public function db_read()
	{
		if ($this->tank_auth->is_logged_in()) {
			if( isset( $_POST['id'])
				...
				$air_invoice_ar = $this->tair_invoice_ar->read( $_POST['id']);
				if( ! empty( $air_invoice_ar )){
					$air_invoice_ar['charges'] = $this->tair_invoice_ar_charges->load( $_POST['id']);
					echo serialize( $air_invoice_ar );
				}
			}
		}
	}
	...
	public function db_update()
	{
		$resulttrn['status']='Berhasil';
		$resulttrn['ket']='';
		if( in_array( 'update_air_invoice_ar', unserialize( $this->tank_auth->get_capabilities() ) )){
			$this->load->library( 'Emkl', 'emkl' );
			if ($this->tank_auth->is_logged_in()) {
				if( isset( $_POST['invoice_no'] )
					&& isset( $_POST['invoice_date'] )
					...
					if ($resulttrn['status']=='Error'){
						$this->xlog->record( 'update', 'air_invoice_ar', $resulttrn );
						echo serialize($resulttrn);
						exit;
					}
					$resulttrnx['isi']=$_POST;
					...
					$hasil=$this->tair_invoice_ar->update( $data );
					if ($hasil=='1'){
						$hasil=true;
						$this->tair_invoice_ar_charges->clean( $_POST['invoice_no']);
						if (isset($air_invoice_charges) && !empty($air_invoice_charges)){
		...
		$resulttrnx['status']=$resulttrn['status'];
		$resulttrnx['ket']=$resulttrn['ket'];
		$this->xlog->record( 'update', 'air_invoice_ar', $resulttrnx );
		echo serialize($resulttrn);
	}
	...
	public function db_delete()
	{
		$resulttrn['status']='Berhasil';
		$resulttrn['ket']='';
		if( in_array( 'delete_air_invoice_ar', unserialize( $this->tank_auth->get_capabilities()))){
			if ($this->tank_auth->is_logged_in()) {
				if( isset( $_POST['id'])
					&& !empty( $_POST['id'])
				){
					if ($this->tair_invoice_ar->delete( $_POST['id'])){
						$this->tair_invoice_ar_charges->clean( $_POST['id']);
					}else{
						$resulttrn['status']='Error';
		...
		$resulttrnx['ket']=$resulttrn['ket'];
		$resulttrnx['isi']=$_POST;
		$this->xlog->record( 'delete', 'air_invoice_ar', $resulttrnx);
		echo serialize($resulttrn);
	}
...
/* End of file air_invoice_ar.php */
/* Location: ./application/controllers/air_invoice_ar.php */
