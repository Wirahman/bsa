...
class Air_import_master extends CI_Controller{
    private $controller = 'transaksi/air_import_master';

	function __construct()
	{
		parent::__construct();
		$this->load->model( 'Tair_import_master', 'tair_import_master');
		$this->load->model( 'Tair_import_master_route', 'tair_import_master_route');
	...
	public function index()
	{
		if (!$this->tank_auth->is_logged_in()) {
			...
			if( in_array( 'read_air_import_master', unserialize( $this->tank_auth->get_capabilities()))){
				$this->xlog->record( 'open', 'Air_import_master', '');
				$this->load->view('transaksi/air_import_master/index', $data);
			}else{
	...
	public function db_air_import_master()
	{
		if ($this->tank_auth->is_logged_in()) {
			if( isset( $_POST[ 'key' ])){
				$this->load->model( 'Search', 'search' );
				$hasil = $this->search->search_air_import_master( $_POST['key']);
				echo serialize( $hasil );
			}
		}
	}
	...
	public function db_lastid()
	{
		if ($this->tank_auth->is_logged_in()) {
			if( isset( $_POST[ 'tanggal' ] ) ){
				echo $this->tair_import_master->lastid( $_POST['tanggal'] );
			}
		}
	}
	public function db_create()
	{
		...
		$resulttrn['ket']='';
		if( in_array( 'create_air_import_master', unserialize( $this->tank_auth->get_capabilities() ) )){
			$this->load->library( 'Emkl', 'emkl' );
			if ($this->tank_auth->is_logged_in()) {
				if( isset( $_POST['MAWB_no'] )
					&& isset( $_POST['order_date'] )
				){
					$baris=1;
					if (empty($_POST['MAWB_no']) ){
						$resulttrn['status']='Error';
						$resulttrn['ket']='MAWB Number <strong>MUST</strong> be filled.';
					...
					if ($resulttrn['status']=='Error'){
						$this->xlog->record( 'create', 'air_import_master', $resulttrn);
					...
					$hasil=$this->tair_import_master->create ( $data );
					if ($hasil=='ada'){
						$resulttrn['status']='Error';
						$resulttrn['ket']='Data Already exist.';
					}elseif ($hasil=='1'){
						$hasil=true;
						$this->tair_import_master_route->clean( $_POST['MAWB_no']);
						if (isset($air_import_route) && !empty($air_import_route)){
							if (!$this->tair_import_master_route->create( $air_import_route)){
								$hasil=false;
							}
						}
						if ($hasil){
							...
							$this->tair_import_master->delete( $_POST['MAWB_no']);
							$this->tair_import_master_route->clean( $_POST['MAWB_no']);
						}
					}else{
						$resulttrn['status']='Error';
		...
		$this->xlog->record( 'create', 'air_import_master', $resulttrnx);
	...
	public function db_read()
	{
		if ($this->tank_auth->is_logged_in()) {
			if( isset( $_POST['id'] )
				...
				$air_import = $this->tair_import_master->read( $_POST['id']);
				if( ! empty( $air_import )){
					$air_import['route'] = $this->tair_import_master_route->load( $_POST['id']);
					echo serialize( $air_import );
				}
			}
		}
	}
	...
	public function db_update()
	{
		$resulttrn['status']='Berhasil';
		$resulttrn['ket']='';
		if( in_array( 'update_air_import_master', unserialize( $this->tank_auth->get_capabilities() ) )){
			$this->load->library( 'Emkl', 'emkl' );
			if ($this->tank_auth->is_logged_in()) {
				if( isset( $_POST['MAWB_no'] )
					&& isset( $_POST['order_date'] )
					...
					if ($resulttrn['status']=='Error'){
						$this->xlog->record( 'update', 'air_import_master', $resulttrn );
						echo serialize($resulttrn);
						exit;
					}
					$resulttrnx['isi']=$_POST;
					...
					$hasil=$this->tair_import_master->update( $data );
					if ($hasil=='1'){
						$hasil=true;
						$this->tair_import_master_route->clean( $_POST['MAWB_no']);
						if (isset($air_import_route) && !empty($air_import_route)){
		...
		$resulttrnx['status']=$resulttrn['status'];
		$resulttrnx['ket']=$resulttrn['ket'];
		$this->xlog->record( 'update', 'air_import_master', $resulttrnx );
		echo serialize($resulttrn);
	}
	...
	public function db_delete()
	{
		$resulttrn['status']='Berhasil';
		$resulttrn['ket']='';
		if( in_array( 'delete_air_import_master', unserialize( $this->tank_auth->get_capabilities() ) )){
			...
			if ($this->tank_auth->is_logged_in()) {
				if( isset( $_POST['id'] )
					...
					if ($this->tair_import_master->delete( $_POST['id'] )){
						$this->tair_import_master_route->clean( $_POST['id']);
					}else{
						$resulttrn['status']='Error';
			...
			$resulttrnx['ket']=$resulttrn['ket'];
			$resulttrnx['isi']=$_POST;
			$this->xlog->record( 'delete', 'air_import_master', $resulttrnx );
			echo serialize($resulttrn);
	}
...
/* End of file air_import.php */
/* Location: ./application/controllers/air_import.php */
